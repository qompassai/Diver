...
local function get_col_num_rfc_lines(line, delim, expected_num_fields)
    ...
    local start_line, end_line = unpack(find_unbalanced_lines_around(cur_line_content))
...
local function converged_select(table_buf_number, rb_script_path, query_buf_nr)
    ...
    local table_filetype = vim.bo[table_buf_number].filetype
    ...
    if meta_language == 'js' then
        ...
        psv_query_status, psv_error_report, psv_warning_report, psv_dst_table_path =
            unpack(M.parse_report(report_content))
...
M.buffer_enable_rainbow_features = function()
    ...
    if num_fields == 1 then
        if vim.g.disable_rainbow_statusline ~= 1 then
            ...
            cnoreabbrev <expr> <buffer> update luaeval('require("rainbow_csv.fns").set_statusline_columns("Update")')
            cnoreabbrev <expr> <buffer> UPDATE luaeval('require("rainbow_csv.fns").set_statusline_columns("Update")')
               cnoreabbrev <expr> <buffer> select getcmdtype() == ':' && getcmdline() =~# '^\s*select\s*$' ? 'Select' : 'select'
            cnoreabbrev <expr> <buffer> update getcmdtype() == ':' && getcmdline() =~# '^\s*update\s*$' ? 'Update' : 'update'
        ]],
                {}
            )
        end
