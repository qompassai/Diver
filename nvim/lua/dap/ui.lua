...
function M.apply_winopts(win, opts)
	...
	assert(type(opts) == 'table', 'winopts must be a table, not ' .. type(opts) .. ': ' .. vim.inspect(opts))
...
function M.trigger_actions(opts)
	...
	if opts.filter then
		local filter = (
			type(opts.filter) == 'function' and opts.filter or function(x)
...
function M.layer(buf)
	...
	layer = {
		...
		render = function(xs, render_fn, context, start, end_)
			...
			local lines = vim.tbl_map(function()
				return ''
			end, xs)
			...
			for i = start, start + #lines - 1 do
				...
				if hl_regions then
					for _, hl_region in pairs(hl_regions) do
						api.nvim_buf_add_highlight(buf, nshl, hl_region[1], i, hl_region[2], hl_region[3])
					end
				end
