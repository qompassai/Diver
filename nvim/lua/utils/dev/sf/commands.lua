...
-- --------------------------------------------------
local agent = require('utils.dev.sf.agent')
local analyzer = require('utils.dev.sf.analyzer')
local apex = require('utils.dev.sf.apex')
local auth = require('utils.dev.sf.auth')
local community = require('utils.dev.sf.community')
local data = require('utils.dev.sf.data')
local files = require('utils.dev.sf.files')
local flow = require('utils.dev.sf.flow')
local limits = require('utils.dev.sf.limits')
local org = require('utils.dev.sf.org')
local pkg = require('utils.dev.sf.package')
local query = require('utils.dev.sf.query')
local schema = require('utils.dev.sf.schema')
local tests = require('utils.dev.sf.tests')
local user = require('utils.dev.sf.user')
local M = {}
...
local COMMANDS = {
  ...
  },
  {
    ...
    opts = {
      desc = 'Add a file to an Agentforce Data Library',
      nargs = '?',
    },
  },
  {
    ...
    opts = {
      desc = 'Delete a file from an Agentforce Data Library',
      nargs = '?',
    },
  },
  {
    ...
    opts = {
      desc = 'List files in an Agentforce Data Library',
      nargs = '?',
    },
  },
  {
  ...
  },
  {
    ...
    opts = {
      desc = 'Show org record count limits',
      nargs = 0,
    },
  },
  {
  ...
  },
  {
    ...
    opts = {
      desc = 'Create Salesforce users from a file',
      nargs = '?',
    },
  },
  {
    ...
    opts = {
      desc = 'Generate a password for a user',
      nargs = '?',
    },
  },
  {
...
function M.setup()
  for _, cmd in ipairs(COMMANDS) do
    print(cmd.name, type(cmd.fn))
    if type(cmd.fn) == 'function' or type(cmd.fn) == 'string' then
