...
function M.create()
  ...
  util.open_term_cmd(
    'sf package create --name '
      .. util.shellescape(name)
      .. ' --package-type '
      .. util.shellescape(ptype)
      .. ' --path '
...
function M.version_create()
  ...
  util.open_term_cmd(
    'sf package version create --package '
      .. util.shellescape(pkg)
      .. ' --installation-key-bypass --wait '
      .. util.shellescape(wait),
    tonumber(wait) + 5
  )
end
function M.version_create_status()
  ...
  util.open_term_cmd('sf package version create report --package-create-request-id ' .. util.shellescape(job_id), 20)
end
function M.install()
  ...
  util.open_term_cmd('sf package install --package ' .. util.shellescape(version_id) .. ' --wait 10 --no-prompt', 20)
end
function M.list()
  if not util.ensure_sf() then
    return
...
function M.uninstall()
  ...
  util.open_term_cmd('sf package uninstall --package ' .. util.shellescape(pkg_id) .. ' --wait 10', 20)
end
