...
-- The server list is a plain table; the enable call is deferred to the first
-- FileType event (see bottom of file) instead of running at startup.
local servers = {
    ...
    --'delphi_ls' ---:TODO
    ---@source https://docwiki.embarcadero.com/RADStudio/Florence/en/Code_Insight_Reference
    ...
    'gn_ls', ---:TODO validate
    -- os(https://github.com/google/gn-language-server) vs msft(https://github.com/microsoft/gnls)
    'gop_ls',
    --'gradle_ls',
    ...
    ---@source https://github.com/unisonweb/unison/blob/trunk/docs/language-server.markdown
...
-- Deferred enable: resolving all 227 configs eagerly costs ~35 ms warm /
-- ~165 ms cold (each name triggers an rtp scan plus loadfile in
-- vim.lsp.config.__index), yet no server can start before the first
-- FileType event anyway (clients start only from the FileType handler,
-- lsp_enable_callback in $VIMRUNTIME/lua/vim/lsp.lua). Deferring keeps the
-- enabled set, the autocmds, and the per-buffer behavior identical; the
-- cost moves from startup to the first opened file. enable() itself
-- re-fires FileType for existing buffers (doautoall), so the buffer that
-- triggers this still gets its servers. A config error that eager enable
-- would raise at startup now surfaces here instead, and is reported.
local enable_group = vim.api.nvim_create_augroup('diver.lsp.lazy_enable', { clear = true })
vim.api.nvim_create_autocmd('FileType', {
    group = enable_group,
    desc = 'Enable diver LSP servers on first FileType (deferred from startup)',
    callback = function()
        vim.api.nvim_del_augroup_by_id(enable_group)
        local ok, err = pcall(vim.lsp.enable, servers)
        if not ok then
            vim.notify(string.format('[diver] deferred vim.lsp.enable failed: %s', tostring(err)), vim.log.levels.ERROR)
        end
    end,
})
...
--'sparq_ls' ---outdated
---@source https://github.com/stardog-union/stardog-language-servers/tree/master/packages/sparql-language-server
...
--'turtle_ls', ---outdated
---@source https://github.com/stardog-union/stardog-language-servers/tree/master/packages/turtle-language-server
