--- Rust FFI self-test — proves the Rust bridge loads.
---
--- Plain-language version: FFI (foreign function interface) is how Lua calls into compiled Rust code. This is a
--- test script that proves the bridge loads and answers. It runs headless; it is not loaded at startup.
---@module 'tests.rust-ffi'
local T = {}
...
local rust_path = debug.getinfo(1).source:match('@?(.*/)')
vim.opt.runtimepath:append(rust_path .. '../../rust')
...
_G['_WEBSOCKET_NVIM'].servers.callbacks[server_info.id] = {
    on_message = function(server_id, server_client_id, message)
        print('Server received message', message, 'from client', server_client_id)
        -- 'Hi from client N' greetings are acknowledged without a reply; only
        -- broadcast and disconnect requests get handling.
        if message:match('^Please broadcast$') then
            websocket_server_ffi.broadcast_data(server_id, 'Broadcast from server')
        elseif message:match('^Please disconnect$') then
            websocket_server_ffi.disconnect_client(server_id, server_client_id)
        end
    end,
    ...
    on_client_connect = function(server_id, server_client_id)
        print(
            ...
            server_id,
            'Server clients:',
            vim.inspect(websocket_server_ffi.get_clients(server_id))
        )

        websocket_server_ffi.send_data_to_client(server_id, server_client_id, 'Reply from server')
    end,
    on_error = function(_server_id, error)
...
websocket_client_ffi.connect(client_1_info.id, ('ws://localhost:%d'):format(server_info.port), {
...
websocket_client_ffi.send_data(client_1_info.id, 'Hi from client 1')
...
websocket_client_ffi.connect(client_2_info.id, ('ws://localhost:%d'):format(server_info.port), {
    ['Extra-test-header'] = 'test',
})
...
websocket_client_ffi.send_data(client_2_info.id, 'Hi from client 2')
