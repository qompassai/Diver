#!/usr/bin/env lua
--- Schema generator — builds JSON schemas for this config's files.
---
--- Plain-language version: a JSON schema is a rulebook that describes what a config file is allowed to contain, so
--- editors can autocomplete and validate it. This script generates those rulebooks from the config's own
--- definitions. It runs as a standalone script when schemas need rebuilding.
---@module 'schemagen'
-- schemagen.lua
-- Qompass AI Diver Schema Generation
...
local index = {
    ...
    basedpyright = 'https://raw.githubusercontent.com/DetachHead/basedpyright/main/packages/'
        .. 'vscode-pyright/package.json',
    ...
    tailwindcss = 'https://raw.githubusercontent.com/tailwindlabs/tailwindcss-intellisense/master/packages/'
        .. 'vscode-tailwindcss/package.json',
    ...
    ts_ls = 'https://raw.githubusercontent.com/microsoft/vscode/main/extensions/'
        .. 'typescript-language-features/package.json',
...
local function request(url)
    ...
    vim.net.request(url, {}, function(request_err, response)
        err = request_err
...
local function translate_schema_docs(props, nls_url)
    local localization = vim.json.decode(request(nls_url), {
