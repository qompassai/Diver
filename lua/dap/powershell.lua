...
---@type string[]
local PSES_ROOTS = {
    fs.joinpath(fn.expand('~'), '.local', 'share', 'powershell-editor-services'),

    fs.joinpath(fn.expand('~'), '.local', 'share', 'PowerShellEditorServices'),
...
local function project_root(bufnr)
    ...
    if current ~= '' then
        local root = fs.root(current, ROOT_MARKERS)
...
local function resolve_pwsh()
    if state.pwsh ~= nil and fn.executable(state.pwsh) == 1 then
        return state.pwsh
    end

    local configured = vim.env.NVIM_PWSH_EXECUTABLE

    if nonempty_string(configured) then
        local path = normalize(fn.expand(configured))
        ...
        notify(('NVIM_PWSH_EXECUTABLE is not executable: %s'):format(path), levels.WARN)
    end
...
local function start_script_from_root(root)
    local candidates = {
        fs.joinpath(root, 'PowerShellEditorServices', 'Start-EditorServices.ps1'),

        fs.joinpath(root, 'Start-EditorServices.ps1'),

        fs.joinpath(root, 'modules', 'PowerShellEditorServices', 'Start-EditorServices.ps1'),
    }
...
---@return string?
local function bundle_from_vscode_root(extension_root)
    if not is_directory(extension_root) then
        return nil
    end

    local handle = uv.fs_scandir(extension_root)
    ...
    while true do
        ...
        if kind == 'directory' and name:match('^ms%-vscode%.powershell%-') then
            candidates[#candidates + 1] = fs.joinpath(extension_root, name, 'modules')
        end
    end
    ...
    for _, candidate in ipairs(candidates) do
        if start_script_from_root(candidate) ~= nil then
            return fs.normalize(candidate)
        end
    end
...
local function resolve_bundle()
    if state.bundle ~= nil and start_script_from_root(state.bundle) ~= nil then
        return state.bundle
    end

    local configured = vim.env.NVIM_PSES_BUNDLE or vim.env.POWERSHELL_EDITOR_SERVICES_PATH

    if nonempty_string(configured) then
        local root = normalize(fn.expand(configured))

        if start_script_from_root(root) ~= nil then
            state.bundle = root

            return root
        end

        notify(('configured PSES bundle is invalid: %s'):format(root), levels.WARN)
    end
    ...
    for _, root in ipairs(VSCODE_EXTENSION_ROOTS) do
        local bundle = bundle_from_vscode_root(root)
...
local function session_directory()
    if state.session_dir ~= nil and is_directory(state.session_dir) then
        return state.session_dir
    end

    local base = fs.joinpath(fn.stdpath('cache'), 'dap', 'powershell')
    ...
    state.session_dir = fs.normalize(directory)
...
local function prompt_script_args()
    local input = fn.input('PowerShell script arguments: ')
...
local function prompt_runspace_id()
    local value = fn.input('PowerShell runspace ID: ', '1')

    local id = tonumber(value)

    if id == nil or id < 1 then
        notify(('invalid runspace ID: %s'):format(value), levels.ERROR)
    ...
    for line in (result.stdout or ''):gmatch('[^\r\n]+') do
        ...
        if pid ~= nil and command ~= nil then
            ...
            if
                ...
                or lower_arguments:find('pwsh', 1, true) ~= nil
            then
...
local function select_process()
    ...
    if #processes == 0 then
        notify('no PowerShell host processes found', levels.WARN)
...
local function prompt_process_id()
    local value = fn.input('PowerShell process ID: ')
...
local function adapter_args()
    ...
    local start_script = start_script_from_root(bundle)

    if start_script == nil then
        return {}
    end

    local temporary = session_directory()
    ...
    return {
        ...
        '-LogPath',
        fs.joinpath(temporary, 'powershell-dap.log'),

        '-SessionDetailsPath',
        fs.joinpath(temporary, 'session.json'),
...
local function health()
    ...
    if bundle ~= nil then
        start_script = start_script_from_root(bundle)
    end
    ...
    notify(
        table.concat({
            ...
            'pwsh: ' .. (pwsh or 'not found'),

            'PSES bundle: ' .. (bundle or 'not found'),

            'Start-EditorServices.ps1: ' .. (start_script or 'not found'),
        ...
        (pwsh ~= nil and start_script ~= nil) and levels.INFO or levels.WARN
    )

    return pwsh ~= nil and start_script ~= nil
end

local function select_pwsh()
    local selected = fn.input('PowerShell executable: ', resolve_pwsh() or '', 'file')
    ...
    notify(('PowerShell executable: %s'):format(selected))
end

local function select_bundle()
    local selected = fn.input('PowerShell Editor Services bundle: ', resolve_bundle() or '', 'dir')
    ...
    if start_script_from_root(selected) == nil then
        notify(('Start-EditorServices.ps1 was not found under: %s'):format(selected), levels.ERROR)
    ...
    if type(M.adapter) == 'table' then
        M.adapter.args = adapter_args()
    end

    notify(('PSES bundle: %s'):format(selected))
end
...
local function clear_cache()
    ...
    notify('PowerShell DAP discovery cache cleared')
end
...
M.commands = {
    ...
    PowerShellDebugHost = {
        callback = function()
            ...
            if pid > 0 then
                notify(('selected PowerShell PID: %d'):format(pid))
            end
        end,
...
M.mappings = {
    ...
    powershell_debug_process = {
        ...
        rhs = function()
            ...
            if pid > 0 then
                notify(('PowerShell host PID: %d'):format(pid))
            end
        end,
...
function M.available()
    return resolve_pwsh() ~= nil and resolve_bundle() ~= nil
end
