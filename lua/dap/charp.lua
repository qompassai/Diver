...
---@param suffix string
---@return string?
local function first_file_with_suffix(directory, suffix)
    ...
    for name, kind in fs.dir(directory) do
        if kind == 'file' and name:sub(-#suffix) == suffix then
            return fs.normalize(fs.joinpath(directory, name))
        end
    end
...
---@param suffix string
---@return string?
local function find_upward_file(start, suffix)
    local directory = start

    while nonempty_string(directory) do
        local found = first_file_with_suffix(directory, suffix)
...
local function resolve_netcoredbg()
    if state.netcoredbg ~= nil and executable(state.netcoredbg) then
        return state.netcoredbg
    end

    local configured = vim.env.NVIM_NETCOREDBG
    ...
    if nonempty_string(configured) then
        ...
        notify(('NVIM_NETCOREDBG is not executable: %s'):format(candidate), levels.WARN)
    end

    local candidate = executable_path('netcoredbg')
...
local function resolve_dotnet()
    if state.dotnet ~= nil and executable(state.dotnet) then
        return state.dotnet
    end

    local configured = vim.env.NVIM_DOTNET_EXECUTABLE
    ...
    if nonempty_string(configured) then
        ...
        notify(('NVIM_DOTNET_EXECUTABLE is not executable: %s'):format(candidate), levels.WARN)
    end

    local candidate = executable_path('dotnet')
...
local function tool_version(executable_name)
    local path = executable_path(executable_name)
...
local function current_project()
    ...
    if state.root == root and state.project ~= nil and is_file(state.project) then
        return state.project
    end

    local current = filename()
    local start = current ~= '' and fs.dirname(current) or root

    if start ~= nil then
        local project = find_upward_file(start, '.csproj')
    ...
    local root_project = first_file_with_suffix(root, '.csproj')
...
local function project_directory()
    ...
    if project ~= nil then
        return fs.dirname(project) or project_root()
    end
...
local function project_name()
    ...
    local name = fs.basename(project):gsub('%.csproj$', '')
    return name
end
...
---@param result string[]
---@param depth integer
local function collect_dlls(root, result, depth)
    if depth > 8 or not is_directory(root) then
        return
    end
    ...
    for name, kind in fs.dir(root) do
        ...
        if kind == 'directory' then
            if name ~= 'obj' and name ~= 'ref' and name ~= 'refint' then
                collect_dlls(path, result, depth + 1)
            end
        elseif kind == 'file' and name:sub(-4) == '.dll' then
...
local function dll_candidates()
    local bin = fs.joinpath(project_directory(), 'bin')
    ...
    collect_dlls(bin, result, 0)

    local expected = project_name() .. '.dll'

    table.sort(result, function(left, right)
        local left_name = fs.basename(left)

        local right_name = fs.basename(right)

        if left_name == expected and right_name ~= expected then
            return true
        end

        if right_name == expected and left_name ~= expected then
            return false
        end
...
local function choose_dll()
    if state.dll ~= nil and is_file(state.dll) then
        return state.dll
    end

    local candidates = dll_candidates()
    ...
    if #candidates > 1 then
        ...
        if selected >= 1 and selected <= #candidates then
            state.dll = candidates[selected]

            return state.dll
        end
    end

    local selected = fn.input('.NET assembly: ', fs.joinpath(project_directory(), 'bin', 'Debug', ''), 'file')
    ...
    if not is_file(selected) then
        notify(('assembly does not exist: %s'):format(selected), levels.ERROR)
...
local function prompt_configuration()
    ...
    for index, configuration in ipairs(BUILD_CONFIGURATIONS) do
        choices[#choices + 1] = ('%d. %s'):format(index, configuration)
    end

    local selected = fn.inputlist(choices)

    if selected >= 1 and selected <= #BUILD_CONFIGURATIONS then
...
local function build_project(configuration)
    ...
    if dotnet == nil then
        notify('dotnet was not found', levels.ERROR)
    ...
    if project == nil then
        notify('no .csproj was found for the current buffer', levels.ERROR)
    ...
    if result == nil then
        notify('failed to invoke dotnet build', levels.ERROR)

        return false
    end

    if result.code ~= 0 then
        local output = vim.trim(result.stderr or result.stdout or 'dotnet build failed')

        notify(output ~= '' and output or 'dotnet build failed', levels.ERROR)
...
local function prompt_pid()
    local input = fn.input('.NET process PID: ')
...
local function dotnet_processes()
    ...
    for line in (result.stdout or ''):gmatch('[^\r\n]+') do
        ...
        if pid ~= nil and command ~= nil then
            ...
            local lower_arguments = (arguments or ''):lower()

            if lower_command == 'dotnet' or lower_arguments:find('dotnet', 1, true) ~= nil then
...
local function choose_dotnet_process()
    local processes = dotnet_processes()

    if #processes == 0 then
        notify('no CoreCLR/dotnet processes found', levels.WARN)
...
local function select_netcoredbg()
    local selected = fn.input('NetCoreDbg executable: ', resolve_netcoredbg() or '', 'file')
    ...
    notify(('NetCoreDbg: %s'):format(selected))
end

local function select_dotnet()
    local selected = fn.input('dotnet executable: ', resolve_dotnet() or '', 'file')
    ...
    notify(('dotnet: %s'):format(selected))
end
...
local function clear_cache()
    ...
    notify('C# DAP discovery cache cleared')
end
...
local function status()
    notify(table.concat({
        ...
        'dotnet: ' .. (resolve_dotnet() or 'not found'),

        'dotnet version: ' .. (tool_version('dotnet') or 'unknown'),

        'netcoredbg: ' .. (resolve_netcoredbg() or 'not found'),

        'cached assembly: ' .. (state.dll or 'none'),
...
M.adapter = {
    ...
    command = resolve_netcoredbg() or 'netcoredbg',
...
M.commands = {
    CSharpDebugBuild = {
        callback = function()
            build_project(prompt_configuration())
        end,
...
function M.available()
    return resolve_netcoredbg() ~= nil and resolve_dotnet() ~= nil
end
