--[[
#################################################################
# /qompassai/lua/dap/breakpoints.lua
# Qompass AI Debug Adapter Protocol (DAP) Breakpoints Module
...
---@param bp dap.bp
---@return string sign name reflecting the breakpoint state
...
function M.set_state(bufnr, state)
    local lnum = as_integer(state.line)
    if not lnum or lnum < 1 then
        return
    end

    local ok, placements = pcall(vim.fn.sign_getplaced, bufnr, {
        group = ns,
        lnum = lnum,
    })
    if not ok then
    ...
    local breakpoints = bp_by_sign_by_buf[bufnr]
    if not breakpoints then
        return
    end
    for _, sign in pairs(signs) do
        local sign_id = as_integer(sign.id)
        if sign_id then
            local bp = breakpoints[sign_id]
            if bp then
                bp.state = state

                if state.verified == false then
                    vim.fn.sign_place(sign_id, ns, 'DapBreakpointRejected', bufnr, {
                        lnum = lnum,
                        priority = 21,
                    })
                end
...
function M.toggle(opts, bufnr, lnum)
    ...
    local sign_id = vim.fn.sign_place(0, ns, sign_name, bufnr, {
        lnum = lnum,
        priority = 21,
    })
    if sign_id ~= -1 then
...
---@alias dap.BreakpointsByBuffer table<integer, dap.bp[]>
---@param bufexpr? integer|string
---@return dap.BreakpointsByBuffer
function M.get(bufexpr)
    local signs = get_breakpoint_signs(bufexpr)
    if #signs == 0 then
        return {}
    end
    ---@type dap.BreakpointsByBuffer
    local result = {}
