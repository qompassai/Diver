...
local function prompt_args()
    local input = fn.input('Scala arguments: ')
...
local function prompt_jvm_options()
    local input = fn.input('JVM options: ')
...
local function prompt_env_file()
    ...
    local selected = fn.input('Environment file: ', fs.joinpath(root, '.env'), 'file')
    ...
    if not file(selected) then
        notify(('environment file does not exist: %s'):format(selected), levels.ERROR)
...
local function prompt_main_class()
    return fn.input('Scala main class: ')
end

---@return string
local function prompt_test_class()
    return fn.input('Scala test class: ')
end

---@return string?
local function prompt_build_target()
    local target = fn.input('Metals build target (blank = auto): ')
...
local function prompt_attach_host()
    local host = fn.input('JVM debug host: ', DEFAULT_ATTACH_HOST)
...
local function prompt_attach_port()
    local value = fn.input('JVM debug port: ', tostring(DEFAULT_ATTACH_PORT))

    local port = tonumber(value)

    if port == nil or port < 1 or port > 65535 then
        notify(('invalid JVM debug port: %s'):format(value), levels.ERROR)
...
local function endpoint_uri(value)
    ...
    return value.uri or value.address or value.endpoint
end
...
local function parse_endpoint(uri)
    ...
    local host, port = uri:match('^tcp://([^:/]+):(%d+)/?$')
    ...
    if numeric_port == nil or numeric_port < 1 or numeric_port > 65535 then
        return nil
    end
...
---@param params fun(): table
---@return table
local function adapter(key, params)
    return {
        name = key,
        ...
        port = function()
            return adapter_port(key, params)
        end,
...
local function metals_command(command, arguments)
    local client = metals_client()

    if client == nil then
        notify('Metals LSP is not attached', levels.ERROR)
    ...
    client:request('workspace/executeCommand', {
        ...
        if error ~= nil then
            notify(('%s failed: %s'):format(command, vim.inspect(error)), levels.ERROR)
        end
    end)
end

local function reconnect_build_server()
    metals_command('metals-build-connect')
end
...
local function show_status()
    ...
    local bloop_directory = fs.joinpath(root, '.bloop')

    local bsp_directory = fs.joinpath(root, '.bsp')
    ...
    notify(
        table.concat({
            ...
            '.bsp: ' .. (directory(bsp_directory) and 'present' or 'absent'),

            'session broker: ' .. METALS_COMMAND,

            'last DAP endpoint: ' .. (state.endpoint ~= nil and state.endpoint.uri or 'none'),
        }, '\n'),
        client ~= nil and levels.INFO or levels.WARN
    )
end

---@type table<string, table>
M.adapters = {
    ['scala-run-or-test'] = adapter('scala-run-or-test', current_run_or_test_params),

    ['scala-run'] = adapter('scala-run', current_run_params),

    ['scala-test-file'] = adapter('scala-test-file', current_test_params),

    ['scala-test-target'] = adapter('scala-test-target', test_target_params),

    ['scala-with-args'] = adapter('scala-with-args', current_with_args_params),

    ['scala-with-env'] = adapter('scala-with-env', current_with_env_params),

    ['scala-main-class'] = adapter('scala-main-class', main_class_params),

    ['scala-test-class'] = adapter('scala-test-class', test_class_params),

    ['scala-attach'] = adapter('scala-attach', attach_params),
}
