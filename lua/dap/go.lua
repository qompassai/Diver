...
local function resolve_dlv()
    if state.dlv ~= nil and executable(state.dlv) then
        return state.dlv
    end
    ...
    if nonempty_string(configured) then
        ...
        notify(('NVIM_DLV_EXECUTABLE is not executable: %s'):format(candidate), levels.WARN)
    end

    for _, command in ipairs(DELVE_EXECUTABLES) do
        local candidate = executable_path(command)
...
local function resolve_go()
    if state.go_executable ~= nil and executable(state.go_executable) then
    ...
    if nonempty_string(configured) then
        ...
        notify(('NVIM_GO_EXECUTABLE is not executable: %s'):format(candidate), levels.WARN)
    end

    for _, command in ipairs(GO_EXECUTABLES) do
        local candidate = executable_path(command)
...
local function prompt_args()
    local input = fn.input('Go program arguments: ')
...
local function prompt_test_args()
    local input = fn.input('Go test arguments: ', '-test.v')
...
local function prompt_build_flags()
    local input = fn.input('Go build flags: ')
...
local function prompt_process_prefix()
    return fn.input('Wait for process name prefix: ')
end

---@return string
local function prompt_executable()
    local default = state.executable or fs.joinpath(project_root(), 'bin', '')

    local selected = fn.input('Go executable: ', default, 'file')
    ...
    if not executable(selected) then
        notify(('not an executable file: %s'):format(selected), levels.ERROR)
...
local function prompt_core_file()
    local selected = fn.input('Core dump: ', project_root(), 'file')
    ...
    if not is_file(selected) then
        notify(('core file does not exist: %s'):format(selected), levels.ERROR)
...
local function prompt_trace_directory()
    local selected = fn.input('rr trace directory: ', project_root(), 'dir')
    ...
    if not is_directory(selected) then
        notify(('trace directory does not exist: %s'):format(selected), levels.ERROR)
...
local function prompt_substitute_path()
    local from = fn.input('Local source root: ', project_root(), 'dir')

    if from == '' then
        return {}
    end

    local to = fn.input('Remote source root: ')
...
local function select_dlv()
    local selected = fn.input('Delve executable: ', resolve_dlv() or '', 'file')
    ...
    if type(M.adapter) == 'table' and type(M.adapter.executable) == 'table' then
        M.adapter.executable.command = selected
    end

    notify(('Delve executable: %s'):format(selected))
end

local function select_go()
    local selected = fn.input('Go executable: ', resolve_go() or '', 'file')
    ...
    notify(('Go executable: %s'):format(selected))
end
...
local function clear_cache()
    ...
    notify('Go DAP discovery cache cleared')
end
...
local function status()
    ...
    notify(
        table.concat({
            ...
            'Go version: ' .. (go_version() or 'unknown'),

            'Delve: ' .. (dlv or 'not found'),

            'Delve version:\n' .. (dlv_version() or 'unknown'),

            'cached executable: ' .. (state.executable or 'none'),

            'transport: tcp://127.0.0.1:<ephemeral>',
        }, '\n'),
        (dlv ~= nil and go ~= nil) and levels.INFO or levels.WARN
    )
end
...
function M.available()
    return resolve_go() ~= nil and resolve_dlv() ~= nil
end
