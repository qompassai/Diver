...
local function resizing_layer(win, buf)
  ...
  layer.render = function(...)
    original_render(...)
    if api.nvim_win_is_valid(win) and api.nvim_buf_is_valid(buf) and api.nvim_win_get_config(win).relative ~= '' then
...
function M.cursor_float(widget, winopts)
  local view = M.builder(widget).new_win(M.with_resize(with_winopts(M.new_cursor_anchored_float_win, winopts))).build()
  view.open()
  return view
end
