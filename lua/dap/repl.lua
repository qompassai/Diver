...
---@return integer bufnr of the new REPL prompt buffer
...
---@return integer 0-indexed line number the text was appended at
...
do
    ...
    ---@return table[] completion items for vim.fn.complete
    ...
    ---@param prefix string typed prefix to match against
    ---@return boolean mixed, number? start
        ...
        local function on_response(err, response)
            ...
            elseif response then
                ...
                if start and not mixed then
                    vim.fn.complete(math.floor(offset + start + 1), completions_to_items(items))
                else
