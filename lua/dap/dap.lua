...
---Get the debugger status string.
---@return string status the current debug status text
M.status = function()
...
---@return dap.listeners
local function new_listener_collection()
    local listeners = setmetatable({}, {
        __index = function(tbl, key)
            local callbacks = {}
            rawset(tbl, key, callbacks)
            return callbacks
        end,
    })

    ---@cast listeners dap.listeners
    return listeners
end

M.listeners = {
    before = new_listener_collection(),
    after = new_listener_collection(),
...
do
    ---Runs inside the config-expansion coroutine (see prepare_config); may yield.
    local function eval_option(option)
        ...
        if type(option) == 'thread' then
            ...
            -- Intentional await-in-sync: eval_option is dual-mode (only yields for
            -- thread options); marking it async would falsely taint sync callers.
        ...
        if vim.in_fast_event() then
            ...
            -- Intentional await-in-sync: this runs in a coroutine resumed from
            -- luv threads; marking it async would falsely taint sync callers.
            coroutine.yield()
        end
        return option
    end
    ...
    ---@type fun(config: dap.Configuration): dap.Configuration
    M.listeners.on_config['dap.expand_variable'] = function(config)
        local expanded = vim.tbl_map(expand_config_variables, config)
        ---@cast expanded dap.Configuration
        return expanded
    end
end
...
local function select_config_and_run(opts)
    ...
    lazy.async.run(function()
        ...
        if #all_configs == 0 then
            local msg = 'No configuration found for `%s`. You need to add '
                .. 'configs to `dap.configurations.%s` (See `:h dap-configuration`)'
...
local function prepare_config(config)
    ...
    if mt and type(mt.__call) == 'function' then
        ---@type dap.Configuration the __call contract returns an expanded config table
        local expanded = config()
        assert(expanded ~= nil, 'config metatable __call must return a config table')
        config = expanded
    end
...
---@field filetype? string filetype of the buffer the run was triggered from
...
---Focus the current stack frame in the UI.
---@return nil
...
---Step backwards (reverse debugging).
---@param opts? dap.step.opts?
---@return nil
...
---Pause the given thread, or the current one.
...
---@param cb fun()? legacy positional callback run when termination finishes
---@return nil
...
---Jump execution to the given line.
---@param line? integer
---@return nil
...
---@param lsessions table<number, dap.Session>
...
---Continue execution until the cursor line.
---@return nil
...
---Set the DAP client log level.
---@param level? dap.log.Levels
---@return nil
...
---Tag function for jumping to breakpoint locations.
---@param flags string
---@return any
