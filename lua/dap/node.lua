...
local function adapter_from_root(root)
    ...
    for _, relative in ipairs(SERVER_RELATIVE_PATHS) do
...
local function adapter_from_extension_root(root)
    ...
    for _, name in ipairs(matches) do
        local extension = fs.joinpath(root, name)

        local candidate = adapter_from_root(extension)
...
local function resolve_adapter()
    if state.adapter ~= nil and readable_file(state.adapter) then
    ...
    if nonempty_string(direct) then
        local candidate = normalize(fn.expand(direct))
        ...
        notify(('NVIM_JS_DEBUG_ADAPTER is not readable: %s'):format(candidate), levels.WARN)
    end

    --
    -- Root of a standalone vscode-js-debug build.
    --
    local configured_root = vim.env.NVIM_JS_DEBUG_ROOT

    if nonempty_string(configured_root) then
        local candidate = adapter_from_root(normalize(fn.expand(configured_root)))
    ...
    for _, root in ipairs(VSCODE_EXTENSION_ROOTS) do
        local candidate = adapter_from_extension_root(root)
...
local function prompt_url()
    local value = fn.input('Browser URL: ', DEFAULT_URL)
...
local function prompt_port(prompt, default)
    local input = fn.input(prompt, tostring(default))

    local value = tonumber(input)

    if value == nil or value < 1 or value > 65535 then
        notify(('invalid port: %s'):format(input), levels.WARN)
...
local function node_attach_port()
    return prompt_port('Node inspector port: ', DEFAULT_NODE_PORT)
end

---@return integer
local function browser_attach_port()
    return prompt_port('Browser debugging port: ', DEFAULT_BROWSER_PORT)
end

---@return string
local function prompt_npm_script()
    local value = fn.input('npm script: ', 'dev')
...
---@return string?
local function local_node_bin(root, executable)
    local candidate = fs.joinpath(root, 'node_modules', '.bin', executable)
...
local function typescript_runtime()
    ...
    notify('tsx or ts-node was not found; direct TypeScript launch may fail', levels.WARN)
...
local function current_is_typescript()
    return TYPESCRIPT_FILETYPES[filetype()] == true
end
...
local function browser_executable()
    local configured = vim.env.NVIM_JS_DEBUG_BROWSER
    ...
    for _, command in ipairs(candidates) do
        local executable = executable_path(command)
...
local function show_status()
    ...
    notify(
        table.concat({
            ...
            'TypeScript runtime: ' .. (ts_runtime or 'not found'),

            'adapter: ' .. (adapter or 'not found'),

            'browser: ' .. browser_executable(),
        }, '\n'),
        (adapter ~= nil and node_path ~= nil) and levels.INFO or levels.WARN
    )
end

local function select_adapter()
    local selected = fn.input('js-debug adapter: ', resolve_adapter() or '', 'file')
    ...
    if not readable_file(selected) then
        notify(('not a readable js-debug adapter: %s'):format(selected), levels.ERROR)
    ...
    local function update_adapter(adapter)
        if type(adapter) ~= 'table' or type(adapter.executable) ~= 'table' then
            return
        end
    ...
    update_adapter(M.adapters['pwa-node'])

    update_adapter(M.adapters['pwa-chrome'])

    notify(('js-debug adapter: %s'):format(selected))
end

local function clear_adapter()
    state.adapter = nil

    notify('js-debug adapter path cache cleared')
end
...
local configurations = {
    ...
    --
    {
        ...
        runtimeExecutable = browser_executable,
...
function M.setup(opts)
    ...
    local function configure_adapter(spec)
        if type(spec) ~= 'table' or type(spec.executable) ~= 'table' then
            return
        end

        spec.executable.command = node_path
    ...
    configure_adapter(M.adapters['pwa-node'])

    configure_adapter(M.adapters['pwa-chrome'])
end
