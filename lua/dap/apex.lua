...
---@return string?
local function adapter_from_root(
  root,
  relative_paths
)
...
---@return string?
local function adapter_from_extension_directory(
  prefix,
  adapter_file
)
...
---@param adapter_file string
---@return string?
local function adapter_from_vscode_install(
  adapter_file
)
  for _, root in ipairs(VSCODE_EXTENSION_ROOTS) do
    local candidate =
      adapter_from_extension_directory(
        root,
        adapter_file
      )
...
local function resolve_adapter(
  ...
  if nonempty_string(configured) then
    ...
    if directory(candidate) then
      local nested = adapter_from_root(
        candidate,
        relative_paths
      )
    ...
    notify(
      ("%s does not resolve to a usable adapter: %s"):format(
        env_name,
        candidate
      ),
      levels.WARN
  ...
  --
  -- A locally cloned/built Salesforce monorepo can be used directly.
  --
  local salesforce_root =
    vim.env.NVIM_SALESFORCE_DAP_ROOT

  if nonempty_string(salesforce_root) then
    local candidate = adapter_from_root(
      expand_path(salesforce_root),
      relative_paths
    )
  ...
  --
  -- Fall back to a VS Code/VSCodium extension installation.
  --
  return adapter_from_vscode_install(
    adapter_file
  )
end
...
local function replay_adapter()
  if
    ...
    and readable_file(state.replay_adapter)
  then
...
local function interactive_adapter()
  if
    ...
    and readable_file(state.interactive_adapter)
  then
...
local function required_adapter(
  ...
  notify(
    ("%s adapter was not found"):format(
      description
    ),
    levels.ERROR
  )
  ...
  --
  -- Keep the adapter descriptor structurally valid. Starting it will fail
  -- cleanly instead of causing an immediate Lua nil error.
  --
...
local function choose_log_file()
  ...
  if not readable_file(selected) then
    notify(
      ("debug log does not exist: %s"):format(
        selected
      ),
      levels.ERROR
    )
...
local function prompt_user_ids()
  local input = fn.input(
    "Apex debugger user IDs (comma-separated): "
  )
...
local function prompt_request_types()
  local input = fn.input(
    "Apex request types (comma-separated, blank=all): "
  )
  ...
  for item in input:gmatch("[^,]+") do
    ...
    if VALID_REQUEST_TYPES[value] then
      ...
      notify(
        ("ignoring invalid Apex request type: %s"):format(
          value
        ),
        levels.WARN
      )
...
local function run_sf(arguments)
  ...
  if sf == nil then
    notify(
      "Salesforce CLI `sf` is not installed or is not in PATH",
      levels.ERROR
    )
  ...
  if not ok then
    notify(
      ("Salesforce CLI invocation failed: %s"):format(
        tostring(result)
      ),
      levels.ERROR
    )
...
local function show_sf_orgs()
  ...
  if not nonempty_string(output) then
...
local function show_status()
  ...
  notify(
    table.concat({
      ...
      "sf: "
        .. (sf_path or "not found"),

      "replay adapter: "
        .. (replay or "not found"),

      "interactive adapter: "
        .. (interactive or "not found"),

      "last replay log: "
        .. (state.last_log or "none"),
    }, "\n"),
    (
      node_path ~= nil
      and replay ~= nil
...
local function clear_cached_adapters()
  ...
  notify(
    "Salesforce adapter path cache cleared"
  )
end

local function select_replay_adapter()
  local current = replay_adapter()
    or ""
  ...
  if not readable_file(selected) then
    notify(
      ("not a readable adapter: %s"):format(
        selected
      ),
      levels.ERROR
    )
  ...
  if
    type(M.adapters) == "table"
    and type(M.adapters["apex-replay"]) == "table"
  then
...
local function select_interactive_adapter()
  local current = interactive_adapter()
    or ""

  local selected = fn.input(
    "Apex Interactive adapter: ",
    current,
    "file"
  )
  ...
  if
    type(M.adapters) == "table"
    and type(M.adapters.apex) == "table"
  then
  ...
  notify(
    ("Apex Interactive adapter: %s"):format(
      selected
    )
  )
end
...
--
-- Salesforce publishes both implementations as executable Node DAP
-- programs. They can therefore be launched directly by Neovim without
-- loading VS Code or nvim-dap.
--
...
M.configurations = {
  apex = {
    ...
    --
    -- Salesforce's extension schema requires `logFile`. The replay adapter
    -- performs the actual log simulation locally.
    --
    {
    ...
    --
    -- These fields match Salesforce's currently published DAP schema.
    --
    {
...
function M.setup(opts)
  ...
  state.root = nonempty_string(opts.root)
  ...
  if node_path == nil then
    vim.schedule(function()
      notify(
        "Node.js is required to launch Salesforce Apex debug adapters",
        levels.ERROR
      )
    end)
  ...
  M.adapters["apex-replay"].command =
    node_path

  M.adapters.apex.command =
    node_path
