...
function M.generate_project_files()
    ...
    output.run_with_progress('UnrealGenerateProjectFiles', 'Generating project files', { script, uproject }, {
        show_output = true,
...
function M.package_project()
    ...
    output.run_with_progress('UnrealPackage', 'Packaging (' .. platform .. '/' .. configuration .. ')', {
        uat,
        'BuildCookRun',
        '-project=' .. uproject,
        '-noP4',
...
function M.cook_only()
    ...
    output.run_with_progress('UnrealCookOnly', 'Cooking (' .. platform .. ') -- no build/stage', {
        uat,
        'BuildCookRun',
        '-project=' .. uproject,
        '-noP4',
...
function M.bootstrap_engine_checkout()
    ...
    if vim.fn.filereadable(setup_script) == 0 then
        local prompt_suffix = ' -- is this really an engine source checkout root?'
        notify('Unreal: Setup script not found at ' .. setup_script .. prompt_suffix, levels.ERROR)
        return
    end
    ...
    ---@async
    ---@param task vim.async.Task<vim.SystemCompleted> system task from async_util.system_task()
    ---@return vim.SystemCompleted? completed or nil when the await failed
    local function await_system(task)
        local ok, result = async_util.pawait_task(task)
        if not ok then
            return nil
        end
    ...
    local function bootstrap()
        local setup_completed = await_system(async_util.system_task({ setup_script }, system_opts))
        if setup_completed == nil then
            error('Setup.sh did not complete')
        end
        ...
        local generate_completed = await_system(async_util.system_task({ generate_script }, system_opts))
        if generate_completed == nil then
            error('GenerateProjectFiles.sh did not complete')
        end
        if generate_completed.code ~= 0 then
            error('GenerateProjectFiles.sh exited with code ' .. tostring(generate_completed.code))
        end
    end

    local ok, err = async_util.run_bounded(bootstrap, ENGINE_BOOTSTRAP_TIMEOUT_MS)

    if ok then
        notify(
            'Unreal: engine checkout bootstrapped -- Setup.sh and GenerateProjectFiles.sh both finished.',
            levels.INFO
        )
    else
...
function M.get_actions()
    return {
        ...
        {
            id = 'generate_project_files',
            label = 'Generate project files',
            group = 'Build',
            run = M.generate_project_files,
        },
        {
            id = 'build_project',
            label = 'Build project',
        ...
        {
            id = 'package_project',
            label = 'Package (BuildCookRun)',
            group = 'Package',
            run = M.package_project,
        },
        {
            id = 'package_plugin',
            label = 'Package editor plugin (BuildPlugin)',
            group = 'Package',
            run = M.package_plugin,
        },
        {
            id = 'open_logs',
            label = 'Open Saved/Logs',
            group = 'Project',
            run = M.open_logs,
        },
        {
