...
-- Resolves the first name/path in `candidates` that is executable,
-- honoring $PATH as well as absolute paths. Returns nil if none match.
...
-- Resolves the first readable file/dir in `candidates` (globs allowed).
...
-- Searches upward from `start_dir` (or cwd) for any of the given
-- root marker files/dirs, e.g. {'project.godot'} or {'*.uproject'}.
