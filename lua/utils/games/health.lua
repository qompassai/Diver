--- Games doctor — :checkhealth for the games utilities.
---
--- Plain-language version: `:checkhealth` is Neovim's built-in doctor visit: it checks that everything a feature
--- needs is present. This module is the doctor for the games utilities -- it reports what is installed and what is
--- missing. It runs when you run `:checkhealth utils.games` or `:GamesDoctor`.
---@module 'utils.games.health'
-- #################################################################
...
local function godot_family_probe(module_name)
    return function()
        ...
        if bin ~= nil then
            lines[#lines + 1] = 'reported version: '
                .. (version or 'unavailable (--version did not respond as expected)')
        end
...
local function probe_unity()
    ...
    local version = root and unity.util.project_editor_version(root) or nil
    local editor = root and unity.util.find_editor(root) or nil
    return {
        name = 'Unity',
        ok = editor ~= nil,
...
local function collect_rows()
    ...
    for _, entry in ipairs(completed) do
        ...
        if ok and type(value) == 'table' then
            -- Probes return GamesDoctorRow on success.
            ---@cast value GamesDoctorRow
...
---Run the games-utilities health check and report results.
---@return nil
function M.check()
