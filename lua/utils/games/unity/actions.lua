...
function M.build_target_matrix()
    ...
    local function build_all()
        ...
        for _, target in ipairs(targets) do
            ...
            local await_ok, result = async_util.pawait_task(async_util.system_task(cmd, {
                cwd = root,
                text = true,
            }))
    ...
    local ok, result = async_util.run_bounded(build_all, BUILD_MATRIX_TIMEOUT_MS)
    ...
    for _, entry in ipairs(result) do
        ...
        if not entry.ok then
            quickfix_failures[#quickfix_failures + 1] =
                { filename = entry.log_file, text = entry.target .. ' build failed' }
        end
    end
...
function M.packages_list()
    ...
    for _, package_name in ipairs(package_names) do
        ...
        lines[#lines + 1] =
            string.format('  %-42s requested=%-24s resolved=%s', package_name, requested, resolved_version)
    end
...
function M.packages_add()
    ...
    notify(
        string.format(
            'Unity: added "%s": "%s" to Packages/manifest.json. Open the editor (or run Reimport assets) to'
                .. ' resolve it.',
            package_name,
            version_or_url
        ),
...
function M.describe_project()
    local root = util.find_root()
    local version = root and util.project_editor_version(root) or nil
    local editor = root and util.find_editor(root) or nil
    notify(
        table.concat({
