...
local function batch_export(sheet_flag, extra_args, label)
  ...
  local out =
    shared_util.trim(vim.fn.input('Aseprite export path: ', vim.fn.fnamemodify(sprite, ':r') .. sheet_flag, 'file'))
  if out == '' then
    return
  end
  local cmd = {
  ...
  output.run_with_progress('AsepriteExport', 'Exporting ' .. label .. ' for ' .. vim.fs.basename(sprite), cmd, {
    show_output = true,
...
function M.export_palette()
  ...
  local out =
    shared_util.trim(vim.fn.input('Aseprite palette export path: ', vim.fn.fnamemodify(sprite, ':r') .. '.gpl', 'file'))
  if out == '' then
    return
  end
...
function M.sync_palette_to_user_config()
  ...
  notify(
    'Aseprite: palette synced to ' .. dest .. ' (restart Aseprite, or reload via Edit > Palette > Load Palette).',
    levels.INFO
  )
end
function M.export_sheet_type_preset()
  vim.ui.select(config.sheet_types, {
    ...
    assert(
      vim.tbl_contains(config.sheet_types, sheet_type),
      'Aseprite: unexpected sheet type: ' .. tostring(sheet_type)
    )
    batch_export('.png', function(out)
      local data = vim.fn.fnamemodify(out, ':r') .. '.json'
      return {
...
function M.run_script()
  ...
  output.run_with_progress('AsepriteScript', 'Running script ' .. vim.fs.basename(script), cmd, {
    show_output = true,
...
function M.get_actions()
  return {
    ...
    {
      id = 'export_sprite_sheet',
      label = 'Export sprite sheet + JSON',
      group = 'Export',
      run = M.export_sprite_sheet,
    },
    {
      id = 'export_png_sequence',
      label = 'Export PNG sequence',
      group = 'Export',
      run = M.export_png_sequence,
    },
    {
      id = 'export_gif',
    ...
    {
      id = 'export_sheet_type_preset',
      label = 'Export tileset (choose sheet type)',
      group = 'Export',
      run = M.export_sheet_type_preset,
    },
    {
      id = 'export_palette',
      label = 'Export palette (.gpl)',
      group = 'Palette',
      run = M.export_palette,
    },
    {
      id = 'sync_palette_to_user_config',
      label = 'Sync palette into Aseprite user config',
      group = 'Palette',
      run = M.sync_palette_to_user_config,
    },
    {
      id = 'run_script',
