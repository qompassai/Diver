--- Salesforce core helpers — shared plumbing for the sf modules.
---
--- Plain-language version: this module holds the pieces the other Salesforce modules share: notifications, command
--- registration, and running `sf` CLI commands safely. It is a library other modules call into; it needs the `sf`
--- CLI installed.
---@module 'utils.dev.sf.core'
...
function M.register_commands(specs)
    for _, spec in ipairs(specs) do
        ...
        assert(
            spec.opts == nil or type(spec.opts) == 'table',
            ('Invalid Salesforce command options: %s'):format(spec.name)
        )
