...
local shellescape = core.shellescape
...
function M.create_record(opts)
  ...
  if object_name == '' or values == '' then
    return notify('SObject and values are required', vim.log.levels.WARN)
  end
  local cmd = 'sf data create record --sobject '
    .. shellescape(object_name)
    .. ' --values '
    .. shellescape(values)
    .. org_flag(org)
  run_and_show(cmd, 'sf://data-create-record', 'json')
end
...
function M.get_record(opts)
  ...
  local cmd = 'sf data get record --sobject '
    .. shellescape(object_name)
    .. ' --record-id '
    .. shellescape(record_id)
    .. ' --json'
    .. org_flag(org)
  run_and_show(cmd, 'sf://data-get-record', 'json')
end
...
function M.export_tree(opts)
  ...
  if query == '' then
    return notify('SOQL query or query-file path is required', vim.log.levels.WARN)
  end
  local org = prompt_org()
  local cmd = 'sf data export tree --query ' .. shellescape(query) .. org_flag(org)
  run_and_show(cmd, 'sf://data-export-tree', 'json')
end

function M.query_buffer()
  local lines = api.nvim_buf_get_lines(0, 0, -1, false)
  local query = trim(table.concat(lines, ' '))
  if query == '' then
    return notify('Current buffer contains no SOQL query', vim.log.levels.WARN)
  end
  return M.query({ fargs = { query } })
end
...
function M.query_file(opts)
  ...
  local org = args[2] or prompt_org()
  if path == '' then
    return notify('SOQL file is required', vim.log.levels.WARN)
  end
  local cmd = 'sf data query --file '
    .. shellescape(path)
    .. ' --result-format json'
    .. org_flag(org)
  run_and_show(cmd, 'sf://data-query-file', 'json')
end

M.commands = {
  { name = 'SfDataCreateRecord', fn = M.create_record, opts = { desc = 'Create a Salesforce record', nargs = '*' } },
  { name = 'SfDataDeleteBulk', fn = M.delete_bulk, opts = { desc = 'Bulk-delete Salesforce records', nargs = '*' } },
  { name = 'SfDataDeleteRecord', fn = M.delete_record, opts = { desc = 'Delete a Salesforce record', nargs = '*' } },
  { name = 'SfDataDeleteResume', fn = M.delete_resume, opts = { desc = 'Resume a bulk delete job', nargs = '*' } },
  { name = 'SfDataExportTree', fn = M.export_tree, opts = { desc = 'Export records as an sObject tree', nargs = '*' } },
  { name = 'SfDataGetRecord', fn = M.get_record, opts = { desc = 'Get a Salesforce record', nargs = '*' } },
  { name = 'SfDataImportBulk', fn = M.import_bulk, opts = { desc = 'Bulk-import Salesforce records', nargs = '*' } },
  { name = 'SfDataImportResume', fn = M.import_resume, opts = { desc = 'Resume a bulk import job', nargs = '*' } },
  { name = 'SfDataImportTree', fn = M.import_tree, opts = { desc = 'Import an sObject tree', nargs = '*' } },
  { name = 'SfDataQuery', fn = M.query, opts = { desc = 'Run a SOQL query', nargs = '*' } },
  { name = 'SfDataQueryBuffer', fn = M.query_buffer, opts = { desc = 'Run SOQL from the current buffer', nargs = 0 } },
  { name = 'SfDataQueryFile', fn = M.query_file, opts = { desc = 'Run SOQL from a file', nargs = '*' } },
  { name = 'SfDataResume', fn = M.resume, opts = { desc = 'Resume a Salesforce bulk data job', nargs = '*' } },
  { name = 'SfDataSearch', fn = M.search, opts = { desc = 'Run a SOSL search', nargs = '*' } },
  { name = 'SfDataSoqlFromFile', fn = M.soql_from_file, opts = { desc = 'Generate SOQL from CSV/XLSX headers', nargs = '*' } },
  { name = 'SfDataSoqlFromCurrentFile', fn = M.soql_from_current_file, opts = { desc = 'Generate SOQL from the current CSV/XLSX file', nargs = 0 } },
  { name = 'SfDataUpdateBulk', fn = M.update_bulk, opts = { desc = 'Bulk-update Salesforce records', nargs = '*' } },
  { name = 'SfDataUpdateRecord', fn = M.update_record, opts = { desc = 'Update a Salesforce record', nargs = '*' } },
  { name = 'SfDataUpsertBulk', fn = M.upsert_bulk, opts = { desc = 'Bulk-upsert Salesforce records', nargs = '*' } },
  { name = 'SfDataUpsertResume', fn = M.upsert_resume, opts = { desc = 'Resume a bulk upsert job', nargs = '*' } },
}
