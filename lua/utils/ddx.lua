--- DDX — differential diagnosis: every diagnostic, one searchable list.
---
--- Plain-language version: Neovim collects complaints (diagnostics) from linters and language servers per file. DDX
--- gathers them all -- current buffer, whole workspace, symbols -- into searchable pickers, plus quickfix toggles
--- and self-checks that verify your tooling is healthy. It runs when you invoke its commands; it needs your linters
...
---Run the diagnostics probe and open the report buffer.
---@param opts? table
---@return table
function M.run(opts)
    opts = opts or {}
...
local function tiger_runtime_metrics(metrics, findings, thresholds)
    ...
    if active_handle_count >= thresholds.active_handles then
        add_tiger_finding(
            ...
            1,
            1,
            ('%d active libuv handles exceed the threshold of %d'):format(
                active_handle_count,
    ...
    for _, duplicate in pairs(duplicate_groups) do
        if duplicate.count > 1 then
            add_tiger_finding(
                ...
                duplicate.lnum,
                1,
                ('Autocmd appears %d times: event=%s pattern=%s'):format(
                    duplicate.count,
                    duplicate.event,
                    duplicate.pattern
                ),
                    ...
                    if function_name then
                        ...
                        if previous then
                            add_tiger_finding(
                                ...
                                file_lines,
                                1,
                                ('Function %s redefines the implementation from line %d'):format(
                                    function_name,
                                    previous
                                ),
...
local function tiger_startup_metrics(startup_log, metrics, findings, thresholds)
    ...
    for file, source in pairs(sources) do
        if
            source.self_ms >= thresholds.startup_source_self_ms
            or source.total_ms >= thresholds.startup_source_total_ms
        then
            add_tiger_finding(
                findings,
                'WARN',
...
local function tiger_report_lines(metrics, findings, thresholds)
    ...
    local lines = {
        ...
        ('Largest loaded buffer: %.1f MiB (warn >= %.1f)'):format(
            metrics.largest_buffer_mib or 0,
            thresholds.buffer_mib
        ),
...
function M.selfcheck()
    ...
    write_log(
        fh,
        ('[selfcheck] syntax_ok=%d syntax_err=%d runtime_issues=%d'):format(syntax_ok, syntax_err, runtime_err)
    )
    if fh then
        fh:close()
    end
