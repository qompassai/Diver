...
function M.extract(lines, row, col, filetype)
    ...
    for number, line in ipairs(lines) do
        if number == row then
            cursor = size + math.min(col, #line) + 1
        end
        size = size + #line + 1
        if size > MAX_BUFFER then
            return nil, 'Buffer exceeds the 256 KiB preview limit.'
        end
        local clean = line
        if markdown then
    ...
    while index <= #text do
        ...
        if not escaped(text, index) then
            if pair == '$$' then
                closing, open_length = '$$', 2
            elseif char == '$' then
                closing, open_length = '$', 1
            elseif pair == '\\(' then
                closing, open_length = '\\)', 2
            elseif pair == '\\[' then
                closing, open_length = '\\]', 2
        ...
        if closing then
            ...
            if cursor >= index and cursor <= finish then
                if finish - index + 1 > MAX_EQUATION then
                    return nil, 'Equation exceeds 16 KiB.'
                end
                local equation = original:sub(index, finish)
                if is_environment then
                    return equation
                end
...
local function hide()
    if state.image and img then
        pcall(img.del, state.image)
    end
    state.image = nil
...
local function cancel()
    ...
    if state.job then
        pcall(state.job.kill, state.job, 9)
        state.job = nil
    end
end
...
local function read_file(path, maximum)
    ...
    if not stat or stat.type ~= 'file' or stat.size > maximum then
        return nil
    end
...
local function png_size(bytes)
    ...
    if width < 1 or height < 1 or width > 8192 or height > 8192 then
        return nil
    end
    return width, height
end
...
local function display(bytes)
    if state.buf ~= api.nvim_get_current_buf() then
        return
    end
    local pixels_w, pixels_h = png_size(bytes)
    if not pixels_w or not pixels_h then
        failure('Renderer returned an invalid or oversized PNG.')
        return
    end
    hide()
    ...
    local ok, id = pcall(img.set, bytes, { row = row + 2, col = col + 2, width = width, height = height, zindex = 61 })
    if not ok then
        failure('Native image display failed: ' .. tostring(id))
        return
    end
...
local function sandbox(directory, command)
    local args = { vim.fn.exepath('bwrap'), '--unshare-all', '--die-with-parent', '--new-session', '--clearenv' }
...
local function render(equation)
    ...
    if state.key == key and state.png ~= nil then
        display(state.png)
        return
    end
    cancel()
    ...
    if not directory then
        failure('Cannot create private rendering directory: ' .. tostring(err))
        return
    end
    ...
    if not fd then
        ...
        failure('Cannot write equation.tex')
        return
    end
    local document = M.document(equation)
    local written = uv.fs_write(fd, document, 0)
    uv.fs_close(fd)
    if written ~= #document then
        vim.fn.delete(directory, 'rf')
        failure('Incomplete equation write')
        return
    end
    local function current()
        return bufnr ~= nil and state.generation == generation and state.buf == bufnr and api.nvim_buf_is_valid(bufnr)
    end
    ...
    run({
        ...
        run({
            ...
            cleanup()
            if not bytes then
                failure('PNG missing or larger than 8 MiB.')
                return
            end
...
local function update()
    if not state.buf or state.buf ~= api.nvim_get_current_buf() then
        return
    end
    local count = api.nvim_buf_line_count(state.buf)
    if api.nvim_buf_get_offset(state.buf, count) > MAX_BUFFER then
        failure('Buffer exceeds the 256 KiB preview limit.')
        return
    end
    local cursor = api.nvim_win_get_cursor(0)
    local equation, err =
        M.extract(api.nvim_buf_get_lines(state.buf, 0, -1, false), cursor[1], cursor[2], vim.bo[state.buf].filetype)
...
function M.start()
    ...
    if not ok or type(backend) ~= 'table' or type(backend.set) ~= 'function' or type(backend.del) ~= 'function' then
        vim.notify('This Neovim build lacks native vim.ui.img.', vim.log.levels.ERROR)
        return
    end
    for _, executable in ipairs({ 'bwrap', 'lualatex', 'pdftoppm', 'prlimit' }) do
        if vim.fn.executable('/usr/bin/' .. executable) ~= 1 then
            vim.notify('Missing Arch dependency: /usr/bin/' .. executable, vim.log.levels.ERROR)
            return
        end
    end
    M.stop()
    ...
    if not timer then
        vim.notify('Cannot allocate equation preview timer.', vim.log.levels.ERROR)
        return
    end
...
function M.setup(options)
    ...
    for name, range in pairs({
        ...
        assert(
            type(value) == 'number' and value == math.floor(value) and value >= range[1] and value <= range[2],
            'Invalid latex_preview option: ' .. name
        )
    end
    assert(
        type(opts.cell_aspect) == 'number' and opts.cell_aspect >= 0.5 and opts.cell_aspect <= 4,
        'Invalid cell_aspect'
    )
    ...
    api.nvim_create_user_command(
        'LatexPreview',
        M.start,
        { force = true, desc = 'Start live native equation preview in this buffer' }
    )
    api.nvim_create_user_command('LatexPreviewStop', M.stop, {
        force = true,
        desc = 'Stop native equation preview',
    })
    api.nvim_create_user_command('LatexPreviewToggle', function()
        ...
        force = true,
        desc = 'Toggle live equation preview',
    })
    api.nvim_create_user_command('LatexPreviewLog', function()
        vim.notify(
            state.error or 'No rendering error. Place the cursor inside a complete equation.',
            vim.log.levels.INFO
        )
    end, {
        force = true,
        desc = 'Show the latest equation render error',
    })
end
