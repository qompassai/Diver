--- Zotero helpers — citations from your reference library, in Neovim.
---
--- Plain-language version: Zotero is the app where researchers keep their library of papers. This module pulls from
--- it: pick collections, insert citations while writing, and update your bibliography file. It runs when you invoke
--- its commands; it needs access to your Zotero library.
---@module 'utils.docs.zotero'
-- #################################################################
-- /qompassai/lua/utils/docs/zotero.lua
-- Qompass AI Zotero
...
-- zotcite is an optional external dependency (the zotcite.nvim plugin). When
-- it is not installed this module degrades to a load-safe stub: requiring it
-- never fails, but calling any exported function raises a clear error.
local zotcite_config_ok, zotcite_config = pcall(require, 'zotcite.config')
local zotcite_ok, zotcite = pcall(require, 'zotcite')
if not (zotcite_config_ok and zotcite_ok) then
    local stub = {}
    local function zotero_unavailable()
        error('utils.docs.zotero unavailable: zotcite not installed', 2)
    end
    for _, name in ipairs({
        ...
        stub[name] = zotero_unavailable
    end
    return stub
end

local config = zotcite_config.get_config()
        ...
        if line:find('Path=') == 1 then
            ...
            if vim.uv.fs_access(zprefs, 'r') then
                ...
                for _, pref in pairs(prefs) do
                    ...
                    if pref:find('extensions.zotero.dataDir') and pref:find('extensions.zotero.dataDir') > 0 then
...
---@param cl string[] The list of collections to be searched for citation keys
...
local function get_sql_data(query, sqlfile)
    ...
    if obj.code ~= 0 then
        ...
        if obj.stderr then
            zwarn(string.format('[%d] %s', obj.code, obj.stderr))
        end
        return nil
    end
...
local function get_collections()
    ...
    for _, v in pairs(sql_data) do
        if not collections[v.collectionName] then
            collections[v.collectionName] = {}
        end
...
local function get_items_key_type()
    ...
    entry = {}
    for _, v in pairs(sql_data) do
        if v.typeName ~= 'attachment' and v.typeName ~= 'annotation' and v.typeName ~= 'note' then
            entry[v.itemID] = { zotkey = v.key, etype = v.typeName, added = v.dateAdded, alastnm = '' }
        end
    end
end
...
local function add_most_fields()
    ...
    for _, v in pairs(sql_data) do
        if entry[v.itemID] then
            entry[v.itemID][v.fieldName] = v.value
        end
    end
end
...
local function get_year(e)
    local y
    if e.date then
        y = e.date:match('^([0-9][0-9][0-9][0-9])')
    end
    if not y and e.issueDate then
...
local function calculate_citekeys()
    ...
    for _, v in pairs(entry) do
        ...
        if not v.citekey then
            v.citekey = calculate_ck(v, has_a, has_t, has_y)
        end
    end
end
    ...
    if kt == 'better-bibtex' then
        ...
        for _, v in pairs(entry) do
            if v.citationKey then
                ...
                table.insert(missing_ck, vim.inspect(v.author) .. ' ' .. vim.inspect(v.title))
            end
        end
...
local function get_bib_ref(item, ktype)
    ...
    if config.hook and config.hook.bib_ref then
        e = config.hook.bib_ref(e)
    end
    ...
    if e['etype'] == 'InCollection' and not e['editor'] then
        e['etype'] = 'InBook'
    end
    ...
    for _, aa in pairs({
        ...
        if e[aa] then
            ...
            e[aa] = string.gsub(e[aa], '<span style="font%-variant:small%-caps;">(.-)</span>', '\\\\textsc{%1}')
    ...
    for _, aa in pairs({
        ...
        if e[aa] then
            ...
            table.insert(ref, '  ' .. aa .. ' = {' .. table.concat(names, ' and ') .. '},')
        end
    end
    if e['issued'] then
        local ds = e['issued']
        ...
        if d[1] ~= '0000' then
            ...
            if d[2] ~= '00' then
                table.insert(ref, '  month = {' .. d[2] .. '},')
            end
            if d[3] ~= '00' then
                table.insert(ref, '  day = {' .. d[3] .. '},')
            end
        end
    end
    if e['urldate'] then
        e['urldate'] = string.gsub(e['urldate'], ' .*', '')
    end
    if e['pages'] then
...
function M.update_bib(zkeys, bibf, ktype, verbose)
    ...
    if f then
        if verbose then
            zwarn('zotcite: updating "' .. tostring(bibf) .. '"\n')
        end
        local key = nil
        ...
        if verbose then
            zwarn('zotcite: writing "' .. tostring(bibf) .. '"\n')
        end
    ...
    for k, _ in pairs(entry) do
        if entry[k][field] == key then
            ...
            if #attachments == 0 then
                return nil, 'No attachments found'
            end
            return attachments, ''
        end
    end
...
---Collect every citation key known to the Zotero library.
---@return string[] citations the citation keys
...
---Fetch the reference data for one citation key.
---@param key string the citation key to look up
---@return table|nil ref the reference entry, or nil when unknown
...
function M.get_annotations(key, offset)
    ...
    for _, v in pairs(notes) do
        if v.c then
            ...
            if lang == 'latex' then
                txt = string.format('%s [comment on \\citet[%s%s]{%s}]', txt, s, v.p, ckey)
            elseif lang == 'typst' then
                txt = string.format('%s [comment on #cite(<%s>, supplement: "%s%s")]', txt, ckey, s, v.p)
            else
        ...
        if v.t then
            ...
            elseif lang == 'typst' then
                txt = string.format('#quote[%s] #cite(<%s>, supplement: "%s%s")', txt, ckey, s, v.p)
                table.insert(lines, txt)
            else
...
local notes_to_tex = function(notes)
    ...
    notes = notes:gsub('<span style="text%-decoration: line%-through">(.-)</span>', '\\strikethrough{%1}')
    ...
    notes = notes:gsub('<span class="citation" data%-citation=".-items%%2F(.-)%%.-">', '\\cite{%1}')
...
local notes_to_md = function(notes)
    ...
    notes = notes:gsub('<span style="text%-decoration: line%-through">(.-)</span>', '~~%1~~')
    notes = notes:gsub(' rel="noopener noreferrer nofollow"', '')
    ...
    notes = notes:gsub('<span class="citation" data%-citation=".-items%%2F(.-)%%.-">', '@%1')
    notes = notes:gsub('<li>%s*(.-)%s*</li>', '\n  - %1')
    notes = notes:gsub('<ul>(.-)</ul>', '\n%1\n')
    notes = notes:gsub('<ol>(.-)</ol>', '\n%1\n')
    notes = notes:gsub('<blockquote>%s*(.-)%s*</blockquote>', '\n> %1\n')
...
function M.get_notes(key)
    ...
    local query = 'SELECT parentItemID, note FROM itemNotes WHERE parentItemID = ' .. tostring(key_id)
    local sql_data = get_sql_data(query)
    if not sql_data then
        return
    end
...
function M.get_match(ptrn, d)
    if get_zotero_mtime(config.zotero_sqlite_path) > ztime then
        load_zotero_data()
    end
...
function M.init()
    ...
    if not cite_template then
        cite_template = '{Authors}-{Year}'
    end
    ...
    if config.banned_words then
        ...
        banned_words = { 'a', 'an', 'the', 'some', 'from', 'on', 'in', 'to', 'of', 'do', 'with' }
    end
    ...
    if not config.zotero_sqlite_path then
        ...
        if adir and not config.attach_dir then
            config.attach_dir = adir
        end
        if zdir then
