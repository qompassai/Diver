...
--- WebSocket server wrapper — host live-updating socket connections (FFI bridge).
---
--- Plain-language version: this module wraps a native (Rust) WebSocket server
--- through an FFI bridge so Neovim can host live-updating connections. The
--- bridge is not initialized in this configuration, so the module loads as a
--- fail-closed stub: requiring it never fails, but every use raises a clear
--- "unavailable" error.
---@module 'utils.ux.server'
...
if _G['_WEBSOCKET_NVIM'] == nil then
    ...
    -- fails, yet constructing or using a server raises a clear error, so
    -- nothing silently pretends to work.
    local function server_unavailable()
        error('utils.ux.server unavailable: websocket FFI bridge not initialized', 2)
    end
    --- Stub server class: construction and every method fail closed.
    local WebsocketServerStub = {}
    function WebsocketServerStub.new(_opts)
        server_unavailable()
    end
    setmetatable(WebsocketServerStub, {
        __index = function(_, _)
            return server_unavailable
        end,
    })
    return {
        WebsocketServer = WebsocketServerStub,
        ErrorType = {
...
---@field on_error? fun(server: WebsocketServer, err: WebsocketServerError)
