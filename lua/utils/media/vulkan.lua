...
---Environment for a validation-layer run. The layer path is inherited
---unchanged from the editor's environment (the old shell form did the
---same via $VK_LAYER_PATH expansion).
---@return table<string, string> env
local function validation_env()
    local env = { VK_INSTANCE_LAYERS = 'VK_LAYER_KHRONOS_validation' }
    local layer_path = vim.env.VK_LAYER_PATH
    if layer_path ~= nil then
        env.VK_LAYER_PATH = layer_path
    end
    return env
end

---Run a command in a terminal with the Vulkan validation layer enabled.
---`cmd` is split on plain whitespace into argv and run without a shell,
---so metacharacters in it can never inject extra commands.
---@param cmd string Command line, e.g. './app --flag' (no shell quoting).
function M.run_with_validation(cmd)
    ...
    vim.cmd('botright split')
    local ok_term, term_err = pcall(vim.fn.jobstart, argv, { term = true, env = validation_env() })
    if not ok_term then
        vim.notify('[vulkan-layers] failed to start command: ' .. tostring(term_err), vim.log.levels.ERROR)
    end
end
...
function M.vulkan_run_with_validation(cmd)
    ...
    local ok_term, term_err = pcall(vim.fn.jobstart, argv, { term = true, env = validation_env() })
    if not ok_term then
        vim.notify('[vk-gpu] failed to start command: ' .. tostring(term_err), vim.log.levels.ERROR)
    end
end
