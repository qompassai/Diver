--- Media RPC streams — message plumbing for media tools.
---
--- Plain-language version: RPC (remote procedure call) is how two programs ask each other to do things over a pipe.
--- This module implements the low-level message streams (packed with mpack) that the media tools use to talk. It is
--- plumbing other modules build on.
---@module 'utils.media.rpc'
...
local function coroutine_exec(func, ...)
    ...
    resume_co(coroutine.create(function() ---@async
...
---Dual-mode: yields while the session loop runs (coroutine context);
---otherwise performs a blocking request on the event loop.
function Session:request(method, ...)
    local args = { ... }
    local err, result
    if self._is_running then
        -- Intentional await-in-sync: request is dual-mode (yields only while
        -- the session loop runs); marking it async would falsely taint the
        -- blocking callers.
