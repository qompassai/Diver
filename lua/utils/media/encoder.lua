--- Media encoder — convert audio/video with ffmpeg from Neovim.
---
--- Plain-language version: ffmpeg is the Swiss-army program that converts between audio and video formats. This
--- module builds its command lines for you and runs the conversions. It runs when you invoke its commands; it needs
--- `ffmpeg` installed.
...
local function define_cdefs()
    ...
    int x264_encoder_encode(x264_t *encoder, x264_nal_t **pp_nal, int *pi_nal, x264_picture_t *pic_in,]] .. [[
...
function FFmpeg.create(width, height, fps, output)
    ...
    local cmd = string.format(
        'ffmpeg -y -f rawvideo -pix_fmt rgb24 -s %dx%d -r %d -i pipe:0 -c:v libx264 -preset ultrafast '
...
---@alias EncKind "x264"|"ffmpeg"
---@class media.EncoderModule
---@field create fun(kind: EncKind, width: integer, height: integer, fps: integer, output: string): media.Encoder|nil
local Encoder = {}
