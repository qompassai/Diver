...
---the duckdb wait happens in libuv and cb fires on the main loop
...
function M.query_async(path, sql, opts, cb)
    ...
    if not has_duckdb() then
        fail('duckdb executable was not found on PATH')
        return
    end
    if not path_is_safe(path) then
    ...
    local ok, sysobj = pcall(vim.system, build_argv(path, readonly, { '-json' }), {
        ...
        vim.schedule(function()
            if result.code ~= 0 then
                local err = result.stderr ~= '' and result.stderr or 'duckdb exited with code ' .. result.code
                cb(nil, err)
                return
            end
            local output = (result.stdout or ''):match('^%s*(.-)%s*$')
            ...
            if not dok then
                cb(nil, 'failed to decode duckdb JSON output: ' .. tostring(decoded))
                return
            end
            if type(decoded) ~= 'table' then
                cb(nil, 'duckdb JSON output was not an array')
                return
            end
            cb(decoded, nil)
        end)
    ...
    if not ok then
        fail('duckdb failed to start: ' .. tostring(sysobj))
    end
end
