-- #################################################################
-- qompassai/Diver/lua/config/data/duckdb.lua
-- Qompass AI Diver Native DuckDB Config
-- Copyright (C) 2026 Qompass AI, All rights reserved
-- --------------------------------------------------
--
-- Plugin-free DuckDB tooling for Neovim 0.13+.
--
-- Talks to the `duckdb` CLI directly through vim.system and :terminal.
-- No plugin, no FFI binding. Sibling of lua/config/data/sqlite.lua: same
-- shape, same safety bounds, same command naming convention.
--
-- Two access paths are provided on purpose:
--   1. Human commands (:Duckdb*) for interactive use in a buffer. These
--      follow whatever readonly state the buffer's session was attached
--      with (:DuckdbAttach! for readonly, :DuckdbAttach for read/write).
...
--
-- Commands:
--   :DuckdbAttach[!] [path]  Attach a database file (bang = readonly).
--   :DuckdbDetach            Detach the current buffer's database.
--   :DuckdbRun               Run the buffer, or a visual range, as SQL.
--   :DuckdbTables            List tables in the attached database.
--   :DuckdbSchema [table]    Show schema for the database or one table.
--   :DuckdbTerminal          Open an interactive duckdb REPL in a split.
--   :DuckdbInfo              Show the current buffer's attachment info.
--
-- Agent API:
--   require('config.data.duckdb').query(path, sql, on_result, opts)
--   require('config.data.duckdb').query_sync(path, sql, opts)
--   require('config.data.duckdb').query_buffer(bufnr, sql, on_result, opts)
...
---@class QompassDuckdbConfigOpts
---@field auto_attach? boolean
---@field command_flags? string[]
---@field notify? boolean

---@type QompassDuckdbConfigOpts
M.config = vim.deepcopy(defaults)

---@class QompassDuckdbSession
---@field bufnr integer
---@field path string
---@field readonly boolean
---@field root string

---@class QompassDuckdbQueryOpts
---@field readonly? boolean

M.state = {
        ---@type table<integer, QompassDuckdbSession>
        sessions = {},
}
...
---@param bufnr integer
---@return QompassDuckdbSession|nil
...
---@param opts QompassDuckdbQueryOpts|nil
---@return boolean
...
---@param session QompassDuckdbSession
---@param sql string
---@param title string
        ...
        run_process(session.path, session.readonly, sql, {}, function(result)
                if result.code ~= 0 then
                        local message = result.stderr ~= '' and result.stderr
                                or 'duckdb exited with code ' .. result.code
...
---@param opts? QompassDuckdbQueryOpts
...
local function create_autocmds()
        local group = api.nvim_create_augroup('QompassNativeDuckdb', {
                clear = true,
        })
...
---@param opts? QompassDuckdbConfigOpts
---@return table
