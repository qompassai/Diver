#!/usr/bin/env luajit
--- CSV power tools — rainbow columns, alignment, and SQL-ish queries for CSV files.
---
--- Plain-language version: CSV files (spreadsheets saved as text with commas) are hard to read when the columns do
--- not line up. This module paints each column a different color, lines the columns up, and even lets you run
--- simple select queries against the file as if it were a tiny database. It runs when you open a CSV-ish file; the
--- query features need a Python interpreter for the heavy lifting.
---@module 'config.data.csv'
---@version JIT
local M = {}
...
local function init_groups_from_links()
    ...
    for index, value in ipairs(link_groups) do
        ...
        highlight({ 'link', 'column' .. index - 1, value })
        highlight({ 'link', 'escaped_column' .. index - 1, value })
    end
...
---Initialize the rainbow highlight groups.
---@return nil
...
---Map a CSV dialect to its filetype name.
---@param delim string
---@param policy string
---@param comment_prefix string
---@return string
...
---Parse a CSV filetype name back into a dialect.
---@param ft_val string
---@return table
...
---Generate the rainbow syntax for a dialect if missing.
---@param rainbow_ft string
---@param delim string
---@param policy string
---@param comment_prefix string
---@return nil
M.ensure_syntax_exists = function(rainbow_ft, delim, policy, comment_prefix)
    ...
    writefile(syntax_lines, syntax_file_path)
end
---Register the named CSV dialect filetypes.
---@return nil
...
---Get the dialect of the current buffer.
---@return table
M.get_current_dialect = function()
    return M.ft_to_dialect(vim.o.filetype)
end
---Check whether the current buffer is a rainbow table.
---@return boolean
M.is_rainbow_table = function()
    return M.get_current_dialect()[2] ~= 'monocolumn'
end
---Check rainbow state, tolerating a recent disable.
---@return boolean
...
---Override the python interpreter (debug helper).
...
local function is_comment_line(line, comment_prefix)
    ...
    return vim.startswith(vim.trim(line), comment_prefix)
end

---Split a line, skipping comment lines.
---@param line string
---@param delim string
---@param policy string
---@param comment_prefix string
---@return table, boolean
M.smart_split_with_comments = function(line, delim, policy, comment_prefix)
    if is_comment_line(line, comment_prefix) then
        return {}, false
    end

    return M.preserving_smart_split(line, delim, policy)
end
...
M.find_python_interpreter = function()
    ...
    system_python_interpreter = result.output
...
---Split a line on the delimiter, keeping quotes.
---@param line string
---@param delim string
---@return table, boolean
...
---Split a quoted-policy line into fields.
---@param line string
---@param delim string
---@return string[]
...
---@param line string
---@param preserve_whitespaces boolean
---@return string[]
...
---Split a line according to the given policy.
---@param line string
---@param delim string
---@param policy string
---@return table
M.smart_split = function(line, delim, policy)
    ...
    local splitter = splitters[policy]
    if splitter then
        return splitter()
    else
        notify_err('bad delim policy: ' .. policy)
        return {}
    end
end

---Smart-split while preserving original spacing.
---@param line string
---@param delim string
---@param policy string
---@return table, boolean
...
---Lint the current CSV buffer and show errors.
---@return nil
M.csv_lint = function()
    local delim, policy, comment_prefix = table.unpack(M.get_current_dialect())

    if policy == 'monocolumn' then
        notify_err('CSVLint is available only for highlighted CSV files')
        return
    elseif policy == 'quoted_rfc' then
        -- ✅ Call the RFC implementation instead of showing error
        M.csv_lint_rfc()
        return
    end
    ...
    for linenum = 1, lastLineNo, 1 do
        ...
        if has_warning then
            notify_err(string.format('Line %d has formatting error: double quote chars are not consistent', linenum))
            return
        end
        ...
        if num_fields ~= num_fields_cur then
            notify_err(
                string.format(
                    'Number of fields is not consistent: e.g. line 1 has %d fields, and line %d has %d fields',
                    num_fields,
                    linenum,
                    num_fields_cur
                )
...
---RFC-strict lint of the current CSV buffer.
---@return nil
M.csv_lint_rfc = function()
    local delim = M.get_current_delimiter()
    ...
    for lnum, line in ipairs(lines) do
        if line ~= '' then
            ...
            if has_warning then
                table.insert(errors, {
                    lnum = lnum,
                    message = 'RFC 4180 parsing error: unbalanced quotes',
                })
            end
            if expected_fields == nil then
                expected_fields = #fields
            elseif #fields ~= expected_fields then
                table.insert(errors, {
                    lnum = lnum,
                    message = string.format('Field count mismatch: expected %d, got %d', expected_fields, #fields),
                })
            end
        end
    end
    if #errors == 0 then
        print('CSV lint: No errors found')
    else
        vim.fn.setqflist({}, 'r', {
            title = 'CSV Lint Errors',
            items = vim.tbl_map(function(err)
                return {
...
---Update per-column width stats for one field.
---@param field string
---@param is_first_line boolean
---@param max_field_components_lens table
---@return nil
M.update_subcomponent_stats = function(field, is_first_line, max_field_components_lens)
    ...
    local field_length = vim.api.nvim_strwidth(field)
...
local function display_progress_bar(cur_progress_pos)
    local progress_display_str = 'Processing... ['
        .. string.rep('#', cur_progress_pos)
        .. string.rep(' ', progress_bar_size - cur_progress_pos)
        .. ']'
    vim.cmd.redraw()
    vim.cmd.echo(string.format('%q', progress_display_str))
end

---Adjust column stats for alignment.
---@param column_stats table
---@return table
...
---Pad a field to its column width.
---@param field string
---@param is_first_line boolean
---@param max_field_components_lens table
---@param is_last_column boolean
---@return string
...
---Align the CSV columns in the current buffer.
---@return nil
M.csv_align = function()
    ...
    local column_stats, first_failed_line = table.unpack(calc_column_stats(delim, policy, comment_prefix))
...
---Shrink aligned CSV columns back to compact form.
---@return nil
...
---Read the header row of the current CSV buffer.
---@param delim string
---@param policy string
---@param comment_prefix string
---@return table
M.get_csv_header = function(delim, policy, comment_prefix)
    ...
    for linenum = 1, max_lines_to_check, 1 do
        local line = vim.fn.getline(linenum)
        if comment_prefix == '' or not lua_startswith(line, comment_prefix) then
...
---@param expected_num_fields integer
---@return table
local function do_get_col_num_rfc_lines(cur_line, delim, start_line, end_line, expected_num_fields)
    local record_lines = vim.api.nvim_buf_get_lines(0, start_line, end_line + 1, true)
    ...
    col_num = col_num
        + get_col_num_single_line(
            vim.list_slice(fields, col_num + 1),
            delim,
...
local function get_col_num_rfc_lines(line, delim, expected_num_fields)
    ...
    local start_line, end_line = table.unpack(find_unbalanced_lines_around(cur_line_content))
    local even_number_of_dquotes = #lit_split(line, '"', true) % 2 == 1
    if even_number_of_dquotes then
        if start_line ~= -1 and end_line ~= -1 then
            local report = do_get_col_num_rfc_lines(cur_line_content, delim, start_line, end_line, expected_num_fields)
            if #report > 0 then
                return report
            end
        end
        ...
        if start_line ~= -1 then
            local report =
                do_get_col_num_rfc_lines(cur_line_content, delim, start_line, cur_line_num - 1, expected_num_fields)
            if #report > 0 then
                return report
            end
        end
        if end_line ~= -1 then
            local report =
                do_get_col_num_rfc_lines(cur_line_content, delim, cur_line_num - 1, end_line, expected_num_fields)
            if #report > 0 then
                return report
            end
        end
...
---Show column info for the hovered cell.
---@return nil
M.provide_column_info_on_hover = function()
    if
        rainbow_hover_debounce_ms ~= 0
        and check_debounce('provide_column_info_on_hover', rainbow_hover_debounce_ms, M.provide_column_info_on_hover)
    then
        return
    end
    ...
    local fields
    local col_num
    local _ -- throwaway: unused preserving_smart_split() field
    if policy == 'quoted_rfc' then
        ...
        fields, col_num = table.unpack(report)
    else
...
---Build the statusline text for a tab.
---@param tabstop_val integer
---@param delim_len integer
---@param template_fields table
---@return string[]
M.generate_tab_statusline = function(tabstop_val, delim_len, template_fields)
    ...
    for nf = 1, #template_fields, 1 do
        ...
        local space_filling = string.rep(' ', math.floor(extra_len) + 1)
...
---Set the statusline to show column layout.
---@param eval_value string
---@return string
M.set_statusline_columns = function(eval_value)
    ...
    for nf = 0, num_columns - 1, 1 do
        ...
        rb_statusline = lua_join(
            { rb_statusline, '%#status_color', color_id, '#', column_name, '%#status_line_default_hl#', space_filling },
            ''
        )
    end
    ::done::
    ...
    vim.api.nvim_create_autocmd('CursorMoved', {
        group = vim.api.nvim_create_augroup('StatusDisableGrp', {
            clear = false,
        }),
        pattern = '*',
...
---Run an RBQL SELECT query from a file.
---@return boolean|nil
M.select_from_file = function()
    local delim, policy, _ = table.unpack(M.get_current_dialect())
...
---Parse an RBQL query report.
---@param report_content string
---@return string[] { status, error_report, warning_report, dst_table_path }
...
---@param table_buf_number integer
---@param rb_script_path string
---@param query_buf_nr integer
---@return boolean
local function converged_select(table_buf_number, rb_script_path, query_buf_nr)
    ...
    local table_filetype = vim.bo[table_buf_number].filetype or ''
    ...
    local out_delim, out_policy = table.unpack(get_output_format_params(input_delim, input_policy))
    ...
    if meta_language == 'js' then
        ...
        local cmd_args = {
            'node',
            vim.fn.shellescape(rbql_executable_path),
            vim.fn.shellescape(table_path),
            vim.fn.shellescape(rb_script_path),
            rbql_encoding,
            vim.fn.shellescape(input_delim),
            input_policy,
            vim.fn.shellescape(input_comment_prefix),
            vim.fn.shellescape(out_delim),
            out_policy,
            with_headers_py_tf,
        }
        local cmd = lua_join(cmd_args, ' ')
        local report_content = vim.fn.system(cmd)
        psv_query_status, psv_error_report, psv_warning_report, psv_dst_table_path =
            table.unpack(M.parse_report(report_content))
    elseif system_python_interpreter ~= '' then
        local rbql_executable_path = script_folder_path .. '/rbql_core/vim_rbql.py'
        local cmd_args = {
            system_python_interpreter,
            vim.fn.shellescape(rbql_executable_path),
            vim.fn.shellescape(table_path),
            vim.fn.shellescape(rb_script_path),
            rbql_encoding,
    ...
    if
        vim.fn.index(
            lit_split(psv_warning_report, '\n'),
            'Output has multiple fields: using "CSV" output format instead of "Monocolumn"'
        ) == -1
    then
...
---Associate a table name with the buffer.
...
---Generate rainbow syntax lines for a delimiter.
---@param delim string
---@return string[]
...
---Generate escaped rainbow syntax lines.
---@param delim string
---@return string[]
...
---Generate RFC escaped rainbow syntax lines.
---@param delim string
---@return string[]
...
---Generate whitespace-delimiter syntax lines.
---@return string[]
...
---Apply a rainbow filetype to the buffer.
---@param rainbow_ft string
---@return nil
...
---Set the rainbow filetype for a dialect.
---@param delim string
---@param policy string
---@param comment_prefix string
---@return nil
...
---Disable rainbow features for the buffer.
---@return nil
...
---Enable rainbow features for the buffer.
---@return nil
M.buffer_enable_rainbow_features = function()
    local num_fields = M.get_num_fields()
    ...
    if num_fields == 1 then
        if vim.g.disable_rainbow_statusline ~= 1 then
            ...
            -- The word-boundary abbreviations exceed the line limit inside
            -- a single string, so they are built as locals and run alone.
            local select_abbrev = [[cnoreabbrev <expr> <buffer> select getcmdtype()]]
                .. [[ == ':' && getcmdline() =~# '^\s*select\s*$' ? 'Select' : 'select']]
            local update_abbrev = [[cnoreabbrev <expr> <buffer> update getcmdtype()]]
                .. [[ == ':' && getcmdline() =~# '^\s*update\s*$' ? 'Update' : 'update']]
            vim.api.nvim_exec2(select_abbrev .. '\n' .. update_abbrev, {})
        end
    end
end
...
---Manually set the CSV policy.
---@param arg_policy string
---@param is_multidelim boolean
---@return nil
M.manual_set = function(arg_policy, is_multidelim)
    ...
    if is_multidelim then
        ...
        if #delim > max_delim_len then
            notify_err(
                'Multicharater delimiter is too long. Adjust g:max_multichar_delim_len or use a different separator'
            )
            return
        end
    else
...
---Manually disable CSV rainbow features.
---@return nil
...
---Disable the comment prefix handling.
---@return nil
M.manual_disable_comment_prefix = function()
    notify_err('Not implemented')
end

---Handle a newly created CSV file.
---@return nil
...
-- " TODO fix update -> Update switch; it also occurs with `:echo "update "` ->
-- " `:echo "Update "` scenario, but only with csv files!
...
--             echoerr 'Multicharater comment prefix is too long.'
--             -- Adjust g:max_comment_prefix_len or use a different comment prefix
--             return
--         endif
--     else
