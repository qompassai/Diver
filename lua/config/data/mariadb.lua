--- MariaDB/MySQL companion — run queries without leaving Neovim.
---
--- Plain-language version: this module lets you talk to a MariaDB or MySQL database from inside Neovim: list
--- databases and tables, run a query, and see the results in a buffer. It runs only when you invoke its commands,
--- and it needs a database client installed and a reachable database.
---@module 'config.data.mariadb'
-- #################################################################
...
-- section. See https://mariadb.com/kb/en/configuring-mariadb-with-option-files/
--
...
---@class MariadbConfigOpts
---@field notify? boolean

---@type MariadbConfigOpts
...
---@field host? string
---@field password? string Forbidden: probed by validate_connection and always rejected; use defaults_file instead.
---@field port? integer
---@field socket? string
---@field user? string
...
---@field bufnr integer
---@field conn MariadbConnection
---@field readonly boolean
---@field root string

---@class MariadbQueryOpts
---@field readonly? boolean

M.state = {
    ---@type table<integer, MariadbSession>
    sessions = {},
}
...
---@param opts MariadbQueryOpts|nil
---@return boolean
...
function M.query(conn, sql, on_result, opts)
    ...
    run_process(conn, readonly, sql, { '--batch', '--raw' }, function(result)
        if result.code ~= 0 then
            on_result(nil, result.stderr ~= '' and result.stderr or 'mariadb exited with code ' .. result.code)
            return
        end
...
function M.query_sync(conn, sql, opts)
    ...
    if result.code ~= 0 then
        return nil, result.stderr ~= '' and result.stderr or 'mariadb exited with code ' .. result.code
    end
...
---the mariadb wait happens in libuv and cb fires on the main loop
...
function M.query_async(conn, sql, opts, cb)
    ...
    if not has_mariadb() then
        fail('mariadb executable was not found on PATH')
        return
    end
    local valid, validation_error = validate_connection(conn)
    if not valid then
    ...
    local readonly = resolve_query_readonly(opts)
    local ok, sysobj = pcall(vim.system, build_argv(conn, readonly, { '--batch', '--raw' }), {
        stdin = sql,
        text = true,
        timeout = QUERY_TIMEOUT_MS,
    }, function(result)
        vim.schedule(function()
            if result.code ~= 0 then
                local err = result.stderr ~= '' and result.stderr or 'mariadb exited with code ' .. result.code
                cb(nil, err)
                return
            end
            cb(parse_tsv_rows(result.stdout or ''), nil)
        end)
    end)
    if not ok then
        fail('mariadb failed to start: ' .. tostring(sysobj))
    end
end
...
local function create_autocmds()
    local group = api.nvim_create_augroup('NativeMariadb', {
        clear = true,
    })
