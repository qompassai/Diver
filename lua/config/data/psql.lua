...
--
-- Agent result rows come from --csv output, parsed with a proper RFC4180
-- style parser (quoted fields, embedded commas/newlines, doubled quotes)
...
-- duckdb/mysql siblings place on their structured-output modes.
...
---@class QompassPsqlConfigOpts
---@field notify? boolean

---@type QompassPsqlConfigOpts
M.config = vim.deepcopy(defaults)

---@class QompassPsqlConnection
...
---@class QompassPsqlSession
---@field bufnr integer
---@field conn QompassPsqlConnection
---@field readonly boolean
---@field root string

---@class QompassPsqlQueryOpts
---@field readonly? boolean

M.state = {
        ---@type table<integer, QompassPsqlSession>
        sessions = {},
}
...
---@param conn QompassPsqlConnection
---@return boolean
---@return string|nil
local function validate_connection(conn)
        ...
        -- Deliberately not requiring host/service/passfile: psql's own
        -- defaults (PGHOST, PGUSER, local Unix socket, peer auth) are a
        -- normal and common way to connect, and forcing an explicit field
        -- here would break that legitimate case.
        return true, nil
end

---@param bufnr integer
---@return QompassPsqlSession|nil
...
---@param opts QompassPsqlQueryOpts|nil
---@return boolean
...
---@param session QompassPsqlSession
---@param sql string
---@param title string
        ...
        run_process(session.conn, session.readonly, sql, {}, function(result)
                if result.code ~= 0 then
                        local message = result.stderr ~= '' and result.stderr
                                or 'psql exited with code ' .. result.code
...
---@param opts? QompassPsqlQueryOpts
...
function M.query_sync(conn, sql, opts)
        ...
        local result = vim.system(build_argv(conn, readonly, { '--csv' }), {
                stdin = sql,
                text = true,
                timeout = QUERY_TIMEOUT_MS,
        }):wait()
...
function M.run(opts)
        ...
        run_and_show(
                session,
                sql,
                'psql://' .. (session.conn.database or session.conn.service or 'default') .. ' [result]'
        )
end
...
function M.schema(table_name)
        ...
        local sql = table_name and table_name ~= ''
                and string.format('\\d %s\n', table_name)
                or '\\dt\n'

        run_and_show(
                session,
                sql,
                'psql://' .. (session.conn.database or session.conn.service or 'default') .. ' [schema]'
        )
end
...
---@param args string
---@return QompassPsqlConnection|nil
---@return string|nil
...
local function create_autocmds()
        local group = api.nvim_create_augroup('QompassNativePsql', {
                clear = true,
        })
...
---@param opts? QompassPsqlConfigOpts
---@return table
