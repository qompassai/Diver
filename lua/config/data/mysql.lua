...
---the mysql wait happens in libuv and cb fires on the main loop
...
function M.query_async(conn, sql, opts, cb)
    ...
    if not has_mysql() then
        fail('mysql executable was not found on PATH')
        return
    end
    local valid, validation_error = validate_connection(conn)
    if not valid then
    ...
    local ok, sysobj = pcall(vim.system, build_argv(conn, readonly, { '--batch', '--raw' }), {
        ...
        vim.schedule(function()
            if result.code ~= 0 then
                local err = result.stderr ~= '' and result.stderr or 'mysql exited with code ' .. result.code
                cb(nil, err)
                return
            end
    ...
    if not ok then
        fail('mysql failed to start: ' .. tostring(sysobj))
    end
end
