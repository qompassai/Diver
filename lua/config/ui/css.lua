	...
	local function find_tailwind_root(fname)
		...
		while dir and dir ~= "/" do
			for _, root_file in ipairs(root_files) do
				if util.path.exists(util.path.join(dir, root_file)) then
					return dir
				end
			end
		...
		local tailwind_dir = util.path.join(xdg_config_home, "tailwind")
		for _, root_file in ipairs(root_files) do
			if util.path.exists(util.path.join(tailwind_dir, root_file)) then
				return tailwind_dir
			end
		end
