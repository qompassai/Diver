...
function M.line_rose_status() return rose_status_ok and rose_lualine or nil end
function M.line_lsp()
...
function M.line_datetime()
    ...
    return string.format('%02d/%02d/%04d %02d:%02d:%02d', current_time.month,
                         current_time.day, current_time.year, current_time.hour,
...
function M.line_debounce(func, timeout)
    ...
    return function(...)
        ...
        debounce_timer = vim.fn.timer_start(timeout,
                                            function() func(unpack(args)) end)
    end
end
...
function M.line_extensions(opts)
    opts = opts or {}
    return {
        'aerial', 'ctrlspace', 'fern', 'fugitive', 'fzf', 'lazy', 'man',
        'mason', 'mundo', 'neo-tree', 'nerdtree', 'nvim-dap-ui', 'nvim-tree',
        'oil', 'overseer', 'quickfix', 'symbols-outline', 'toggleterm',
        'trouble'
    }
end
...
function M.line_git_branch(opts)
    opts = opts or {}
    return {'branch', icon = opts.icon or ''}
end
...
M.line_themes = {
    'tokyonight', 'nightfox', 'onedark', 'gruvbox', 'catppuccin', 'everforest',
    'nord', 'dracula', 'material', 'monokai', 'palenight', 'edge', 'darkplus',
    'vscode', 'ayu_dark', 'carbonfox', 'moonfly', 'horizon', 'gotham',
    'github_dark', 'onedarkpro', 'nightowl', 'spacecamp', 'nordfox', 'halcyon',
    'synthwave84', 'matrix', 'vim-monochrome', 'gruvbox-material-dark-hard',
    'sublimemonokai'
}
...
function M.line_settheme(theme_name)
    ...
    require('lualine').setup({options = {theme = theme_name}})
...
function M.line_preview_fzf()
    ...
    require('fzf-lua').fzf_exec(M.line_themes, {
        ...
        actions = {
            ...
            ['ctrl-x'] = function() M.set_theme(current_theme) end
        },
        previewer = false
    })
end
...
function M.line_icon(opts)
    ...
    handlers.filetype = function()
        local icon = custom_icon or
                         require('nvim-web-devicons').get_icon_by_filetype(ft, {
                default = true
            })
        return string.format('%s %s', icon, ft)
    end
    handlers.ft = function() return handlers.filetype() end
    handlers.filename = function()
        local fn = vim.fn.expand('%:t')
        local icon = custom_icon or
                         require('nvim-web-devicons').get_icon(fn, ft,
                                                               {default = true})
        return string.format('%s %s', icon, fn)
    end
    handlers.fn = function() return handlers.filename() end
    handlers.path = function()
        local path = opts.relative and vim.fn.expand('%:.') or
                         vim.fn.expand('%:p')
        local fn = vim.fn.expand('%:t')
        local icon = custom_icon or
                         require('nvim-web-devicons').get_icon(fn, ft,
            ...
            lualine_c = {
                {
                    ...
                    function()
                        local wc = vim.fn.wordcount()
                        return string.format('%d words, %d chars', wc.words,
                                             wc.chars)
                    end,
                    icon = ' '
                }
...
function M.setup_line(opts) M.line_setup(opts) end
return M
