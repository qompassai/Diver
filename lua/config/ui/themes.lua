--- Theme manager — picks and applies your colorscheme.
---
--- Plain-language version: a theme decides all the colors you see -- background, keywords, popups. This module
--- remembers your chosen theme, applies it at startup, and keeps helper plugins (like the Discord presence plugin)
--- in matching colors. It runs at startup and when you switch themes.
...
---Switch to the named theme, applying its highlights.
---@param theme_name string theme to activate
---@return boolean ok true when the theme was applied
...
---@return boolean ok true when the theme was applied
function M.apply_current_theme(_opts)
    return M.set_theme(M.current_theme)
end

---Push the current theme name to the Discord presence plugin.
---@param opts? table option overrides (returned unchanged when cord is unavailable)
---@return table|nil opts the options table, or nil after a successful update
...
---Apply appearance-related options (background, transparency).
---@param _opts? table unused option overrides
---@return table config the effective appearance configuration
function M.setup_appearance(_opts)
    return {
...
---Initialize the Discord presence (cord) plugin.
---@param _opts? table unused option overrides
---@return table cord the cord plugin instance
function M.setup_cord(_opts)
...
---Full theme + cord setup entry point. Idempotent.
