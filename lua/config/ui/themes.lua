...
M.themes = {
    ...
    nord = {
        setup = function() vim.g.nord_transparent = true end,
...
function M.set_theme(theme_name)
    ...
    if not ok_setup then
        vim.notify('Failed to set up theme ' .. theme_name .. ': ' .. err_setup,
    ...
    if M.discord_initialized then M.update_cord_theme() end
    return true
end
function M.apply_current_theme() return M.set_theme(M.current_theme) end
function M.update_cord_theme()
    ...
    M.cord:update_presence({
        custom_details = 'Using ' .. theme.discord_name .. ' theme ' ..
            theme.discord_icon
    })
end
...
function M.setup_text()
    return {
        ...
        workspace = function(opts)
            ...
            local status =
                hour >= 22 and '🌙 Late night coding' or hour >= 18 and
                    '🌆 Evening session' or hour >= 12 and
                    '☀️ Afternoon coding' or hour >= 5 and
                    '🌅 Morning productivity' or '🌙 Midnight hacking'
            return string.format('%s: %s', status,
                                 opts.workspace or 'Unknown project')
        end
    }
end
        ...
        buttons = function(opts)
            ...
            if opts.repo_url then
                table.insert(buttons,
                             {label = 'View Repository', url = opts.repo_url})
            end
            table.insert(buttons,
                         {label = 'Neovim Website', url = 'https://neovim.io'})
            return buttons
        end
    }
end
...
function M.setup_all()
    ...
    if not ok or not cord then
        vim.notify('cord.nvim failed to setup: ' .. tostring(cord),
                   vim.log.levels.ERROR)
        return
    end
    M.cord = cord
