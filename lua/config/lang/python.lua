...
function M.setup_jupyter()
    ...
    vim.keymap.set("n", "<leader>jr", "<cmd>JupyterRunFile<cr>",
                   {desc = "Run Jupyter file"})
    vim.keymap.set("n", "<leader>jc", "<cmd>JupyterSendCell<cr>",
                   {desc = "Send cell"})
    vim.keymap.set("n", "<leader>js", "<cmd>JupyterConnect<cr>",
                   {desc = "Connect Jupyter"})
end
    ...
    vim.api.nvim_create_autocmd("BufEnter", {
        ...
        callback = function()
            local root = vim.fn.findfile("pyproject.toml",
                                         vim.fn.getcwd() .. ";")
            if root ~= "" then
                local venv_py = vim.fn.fnamemodify(root, ":p:h") ..
                                    "/.venv/bin/python"
...
function M.setup_telescope()
    ...
    for _, ext in ipairs({
        "dap", "zoxide", "ui-select", "fzf", "repo", "frecency", "import",
        "conda"
    }) do pcall(telescope.load_extension, ext) end
end
    ...
    vim.api.nvim_create_autocmd("BufRead", {
        ...
        callback = function()
            ...
            if ext == "ipynb" then
                ...
                vim.keymap.set("n", "<leader>x", "<cmd>JupyterSendCell<cr>",
                               {buffer = true, desc = "Run cell"})
                vim.keymap.set("n", "<leader>X", "<cmd>JupyterSendAll<cr>",
                               {buffer = true, desc = "Run all"})
                return
            end
            local markers = {
                "^# %%", "^#%%", "^# In%[", "^// %%", "^//%%", "^// CELL"
