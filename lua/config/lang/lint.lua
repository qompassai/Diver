...
function M.setup_linters(lint)
  ...
  lint.linters.phpstan = {
    ...
    parser = function(output)
      ...
      for filename, file_data in pairs(decoded.files) do
    ...
    parser = function(output)
      ...
      for _, issue in ipairs(decoded.issues or {}) do
        table.insert(diagnostics, {
          ...
          col = (issue.column or 1) - 1,
          message = issue.message,
          severity = issue.type == "error" and vim.diagnostic.severity.ERROR or vim.diagnostic.severity.WARN,
          source = "pine-lint",
        })
      end
      return diagnostics
