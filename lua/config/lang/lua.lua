--- Lua language config — formatters, linters, and helpers for editing Lua.
---
--- Plain-language version: when you open a Lua file, this module sets up the Lua-specific tooling: which formatter
--- to use, which linters to run, and LuaRocks conveniences. It runs on Lua filetypes; the tools it references must
--- be installed to do anything.
...
local function modernize_deprecated_highlights(text)
    local changed = false
    text = text:gsub(
        'vim%.api%.nvim_buf_add_highlight%s*%(%s*([^,]+)%s*,%s*([^,]+)%s*,%s*([^,]+)%s*,'
            .. '%s*([^,]+)%s*,%s*([^,]+)%s*,%s*([^%)]+)%)',
...
---Build the luarocks-aware Lua config by merging user options over defaults.
---@param opts? table option overrides
---@return table cfg the merged Lua config table
...
if vim.lsp and vim.lsp.log and vim.lsp.log.set_level and rawget(vim.lsp, 'set_log_level') == nil then
    -- Compat shim for Neovim builds without vim.lsp.set_log_level. rawget/rawset
    -- keep this from touching the typed (deprecated) vim.lsp field.
    rawset(vim.lsp, 'set_log_level', function(...)
