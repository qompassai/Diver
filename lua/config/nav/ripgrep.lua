...
---Parse ripgrep `--json` output into quickfix items.
---
---Plain-language version: takes the raw text ripgrep printed and turns each
---match line into an entry the quickfix list can show.
---@param output string Raw ripgrep `--json` output to parse.
---@param root string Directory that relative match paths are resolved against.
---@return table[]?, string?
...
function M.search(query, opts)
    ...
    local function run(value)
        ...
        process, err = async.spawn(argv, { cwd = root, timeout = 15000 }, function(result)
            ...
            if not items then
                assert(parse_error ~= nil, 'M.parse returned no items and no error')
                notify(parse_error)
                return
            end
