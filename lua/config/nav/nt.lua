--- Native directory browser — a file explorer built from Neovim's own parts.
---
--- Plain-language version: instead of installing a file-explorer plugin, this module builds one out of Neovim's
--- built-in pieces (new in 0.13): it lists directories, lets you open, create, rename, delete, and move files,
--- shows git status, and can even do privileged (sudo) edits. It runs when you open the browser; everything it
--- needs is already inside Neovim.
---@module 'config.nav.nt'
...
local function windows_command(argv, elevated)
    ...
    local data = { path = argv[#argv], destination = '', value = '', recursive = false, arguments = {} }
    if kind == 'exec' then
...
do
    ...
    local function finished(command, code, options, callback)
        ...
        if code ~= 0 then
            notify(
                'Operation failed or was cancelled (exit ' .. code .. '); inspect its terminal.',
                vim.log.levels.WARN
            )
        end
    ...
    local function windows_browser(root)
        ...
        bootstrap = bootstrap
            .. "require('config.nav.nt').setup({fzf_module="
            .. string.format('%q', M.fzf_module)
            .. '})'
        M.run({
            vim.v.progpath,
...
---Create a directory in the explorer.
---@return nil
...
---Cancel the current explorer operation.
---@return nil
function M.cancel()
    local running = scan
    scan = nil
...
---Show document symbols in the explorer.
---@return nil
function M.symbols()
    source_picker('document_symbols')
end

---Toggle hidden files in the explorer.
---@return nil
...
---Move the selected entries.
---@return nil
function M.move()
    transfer('move')
end
---Create symlinks for the selected entries.
---@return nil
function M.symlink()
    transfer('link')
end
...
---Change permissions of the selected entries.
---@return nil
...
---Change ownership of the selected entries.
---@return nil
...
---Register the explorer user commands.
---@return nil
