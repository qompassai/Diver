--- neo-tree plugin spec — the file-tree sidebar.
---
--- Plain-language version: neo-tree is the sidebar that shows your project's files and folders like a file manager.
--- This file is its instruction card: which plugin to install and how it should look and behave. It is read when
--- the plugin manager sets up plugins.
---@module 'config.nav.neotree'
...
local M = {}
---Build the neo-tree plugin specification table.
---@param _opts? table unused option overrides
---@return nil
function M.neotree_cfg(_opts)
    local neotree_cfg = {
        sources = {
            'filesystem',
            'buffers',
        ...
        source_selector = {
            ...
            -- separator = "|",                                              -- ||  a  |  b  |  c  |...
            separator_active = nil, -- set separators around the active tab.
            -- nil falls back to `source_selector.separator`
        ...
        nesting_rules = {},
        -- Global custom commands that will be available in all sources
        -- (if not overridden in `opts[source_name].commands`)
        --
        ...
        window = { -- see https://github.com/MunifTanjim/nui.nvim/tree/main/lua/nui/popup for
            ...
            -- Affects how nodes get inserted into the tree during
            -- creation/pasting/moving of files if the node under the
            -- cursor is a directory:
            insert_as = 'child',
            ...
            mappings = {
                ['<space>'] = {
                    'toggle_node',
                    -- disable `nowait` if you have existing combos starting with
                    -- this char that you want to use
                    nowait = false,
                },
                ['<2-LeftMouse>'] = 'open',
                ['<cr>'] = 'open',
