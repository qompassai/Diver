...
local function get_linter_spec(name)
    ...
    -- Adapter modules return a `{ unavailable = reason }` sentinel when a
    -- helper dependency failed to load. Skip quietly: there is no spec to
    -- run, and a missing helper is an expected environment condition, not
    -- a config error worth an ERROR notification.
    if type(loaded.unavailable) == 'string' then
        linter_specs[name] = false
        return nil
    end
...
---@param spec Linter
---@return string[]? command, string? cwd, LintContext? context
--- Resolve the executable, argv and cwd for one lint run. Preserves the
--- original diagnostic.reset + notify side effects; returns nil to skip.
local function resolve_lint_invocation(name, bufnr, opts, spec)
    local context = context_for(bufnr, spec)

    local configured_cmd = spec.cmd
    ---@type string|string[]?
    local cmd = type(configured_cmd) ~= 'function' and configured_cmd or nil
    ...
    local executable_name = cmd ~= nil and executable(cmd) or nil
    ...
    return command, cwd_or_error, context
end
...
--- generations, dead buffers and mid-flight edits before publishing.
--- `key` is derived here from bufnr+name, matching run_linter.
local function on_lint_result(name, bufnr, generation, changedtick, spec, context, process, result)
    ...
    if not accepts_exit_code(spec, result.code) and #parsed == 0 then
        ...
        local exit_detail = detail ~= '' and (': ' .. detail) or ''
        vim.notify(('lint: %s failed with exit code %d%s'):format(name, result.code, exit_detail), vim.log.levels.ERROR)
    end
end
...
local function run_linter(name, bufnr, opts)
    ...
    local command, cwd, context = resolve_lint_invocation(name, bufnr, opts, spec)
    if command == nil or cwd == nil or context == nil then
        return false
    end
    ...
    process = vim.system(command, {
        ...
        vim.schedule(function()
            on_lint_result(name, bufnr, generation, changedtick, spec, context, process, result)
        end)
    end)
