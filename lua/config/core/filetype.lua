-- /qompassai/Diver/lua/config/core/filetype.lua
-- Qompass AI Diver Core FileType Config
-- Copyright (C) 2025 Qompass AI, All rights reserved
-- ---------------------------------------------------
local M = {}
local definitions = {
    extension = {
        ada = 'ada',
        adb = 'ada',
        adoc = 'asciidoc',
        ads = 'ada',
        asciidoc = 'asciidoc',
        agda = 'agda',
        antlers = 'antlers',
        apex = 'apex',
        app = 'xml',
        asm = 'asm',
        auradoc = 'xml',
        awk = 'awk',
        axaml = 'xml',
        blade = 'blade',
        bri = 'brioche',
        brioche = 'brioche',
        c = 'c',
        cairo = 'cairo',
        cbl = 'cobol',
        CBL = 'cobol',
        CBLLE = 'cobol',
        cc = 'cpp',
        cdxml = 'xml',
        cff = 'yaml',
        cjs = 'javascript',
        cl = 'opencl',
        clar = 'clar',
        clarity = 'clarity',
        clh = 'opencl',
        cls = 'apex',
        ['cmake.in'] = 'cmake',
        cmp = 'html',
        cobol = 'cobol',
        comp = 'glsl',
        component = 'html',
        cpp = 'cpp',
        cpy = 'cobol',
        CPY = 'cobol',
        cr = 'crystal',
        csproj = 'xml',
        csv = 'csv',
        cxx = 'cpp',
        cypher = 'cypher',
        cython = 'cython',
        dat = 'csv',
        design = 'xml',
        device = 'systemd',
        edge = 'edge',
        ejs = 'ejs',
        erb = 'eruby',
        evt = 'xml',
        f = 'fortran',
        faust = 'faust',
        frag = 'glsl',
        geom = 'glsl',
        go = 'go',
        gohtmltmpl = 'gohtmltmpl',
        gql = 'graphql',
        gradle = 'gradle',
        groff = 'groff',
        h = 'cpp',
        handlebars = 'handlebars',
        hbs = 'html.handlebars',
        hcl = 'hcl',
        hh = 'cpp',
        hpp = 'cpp',
        hs = 'haskell',
        hxx = 'cpp',
        ini = 'dosini',
        intf = 'xml',
        ipynb = 'json',
        j2 = 'jinja',
        jinja2 = 'jinja',
        js = 'javascript',
        jsonld = 'jsonld',
        jsx = 'javascriptreact',
        kts = 'kotlin',
        labels = 'xml',
        ['lagda.md'] = 'agda',
        ['lagda.typ'] = 'agda',
        leaf = 'leaf',
        lua = 'lua',
        m = 'objc',
        markdown = 'markdown',
        md = 'markdown',
        mdx = 'mdx',
        mm = 'objc',
        mjs = 'javascript',
        mojo = 'mojo',
        mount = 'systemd',
        ['🔥'] = 'mojo',
        nftables = 'nftables',
        nginx = 'nginx',
        njk = 'njk',
        nspawn = 'systemd',
        nunjucks = 'nunjucks',
        page = 'visualforce',
        pkgbuild = 'pkgbuild',
        po = 'po',
        pgsql = 'pgsql',
        props = 'xml',
        ps1 = 'ps1',
        ps1xml = 'ps1xml',
        psd1 = 'ps1',
        psql = 'pgsql',
        psrc = 'ps1',
        ql = 'ql',
        qll = 'ql',
        razor = 'razor',
        reST = 'rst',
        rockspec = 'lua',
        rq = 'sparql',
        rst = 'rst',
        scala = 'scala',
        schelp = 'scdoc',
        scss = 'scss',
        sed = 'sed',
        service = 'systemd',
        slim = 'slim',
        snippets = 'snippets',
        soql = 'soql',
        sparql = 'sparql',
        spec = 'dosini',
        sq = 'sparql',
        sugarss = 'sugarss',
        svh = 'systemverilog',
        svn = 'svn',
        tab = 'csv',
        targets = 'xml',
        tesc = 'glsl',
        tese = 'glsl',
        timer = 'systemd',
        tf = 'terraform',
        tfvars = 'terraform',
        tokens = 'xml',
        trig = 'trig',
        trigger = 'apex',
        ts = 'typescript',
        tsv = 'csv',
        tsx = 'typescriptreact',
        ttl = 'turtle',
        -- µcad = 'microcad',
        v = 'verilog',
        vim = 'vim',
        verilog = 'verilog',
        vh = 'verilog',
        x11basic = 'x11basic',
        xaml = 'xaml',
        xml = 'xml',
        xresources = 'xdefaults',
        yaml = 'yaml',
        yml = 'yaml',
        ['yaml.ansible'] = 'yaml.ansible',
        ['yml.ansible'] = 'yaml.ansible',
        yum = 'conf',
        ziggy = 'ziggy',
        ['ziggy-schema'] = 'ziggy_schema',
        zine = 'html',
        zir = 'zir',
    },
    filename = {
        ['ansible.cfg'] = 'ansible',
        ['Appfile'] = 'ruby',
        ['atlas.hcl'] = 'atlas-config',
        ['.bashrc'] = 'bash',
        brioche = 'brioche',
        ['build.zig'] = 'zig',
        ['build.zig.zon'] = 'zon',
        ['buildozer.spec'] = 'dosini',
        Cairo = 'cairo',
        ['CITATION.cff'] = 'yaml',
        ['CMakePresets.json'] = 'json',
        ['CMakeUserPresets.json'] = 'json',
        Crystal = 'crystal',
        ['Dockerfile'] = 'dockerfile',
        ['Containerfile'] = 'dockerfile',
        ['compose.yml'] = 'yaml.docker-compose',
        ['compose.yaml'] = 'yaml.docker-compose',
        ['docker-compose.yml'] = 'yaml.docker-compose',
        ['docker-compose.yaml'] = 'yaml.docker-compose',
        ['.eslintrc.js'] = 'javascript',
        ['.eslintrc.cjs'] = 'javascript',
        ['Fastfile'] = 'ruby',
        ['jupyter_notebook_config.py'] = 'python',
        ['.luacheckrc'] = 'lua',
        ['.luarc.jsonc'] = 'jsonc',
        ['Makefile.local'] = 'make',
        ['make.inc'] = 'make',
        ['Microsoft.PowerShell_profile.ps1'] = 'ps1',
        ['Microsoft.VSCode_profile.ps1'] = 'ps1',
        ['pg_dump.sql'] = 'pgsql',
        ['PKGBUILD'] = 'pkgbuild',
        ['Pluginfile'] = 'ruby',
        ['.powershellrc'] = 'ps1',
        ['powershell.config.json'] = 'jsonc',
        ['profile.ps1'] = 'ps1',
        ['project.bri'] = 'brioche',
        ['ScriptAnalyzerSettings.psd1'] = 'ps1',
        ['stylua.toml'] = 'toml',
        ['tsconfig.json'] = 'jsonc',
        ['Vagrantfile'] = 'ruby',
    },
    pattern = {
        ['.*'] = {
            function(path, bufnr)
                if path:match('%.%w+$') then
                    return nil
                end
                if not bufnr or not vim.api.nvim_buf_is_valid(bufnr) then
                    return nil
                end
                local first = (vim.api.nvim_buf_get_lines(bufnr, 0, 1, false)[1] or ''):sub(1, 4096)
                local env_interp = first:match('^#!%s*/usr/bin/env%s+%-S%s+([%w%._%-]+)')
                    or first:match('^#!%s*/usr/bin/env%s+([%w%._%-]+)')
                local bin_interp = first:match('^#!%s*/.-/([%w%._%-]+)')
                local interp = env_interp or bin_interp
                if not interp then
                    return nil
                end
                local map = {
                    bash = 'bash',
                    fish = 'fish',
                    lua = 'lua',
                    node = 'javascript',
                    php = 'php',
                    pwsh = 'ps1',
                    powershell = 'ps1',
                    python = 'python',
                    python3 = 'python',
                    ruby = 'ruby',
                    sh = 'bash',
                    zsh = 'zsh',
                }
                return map[interp]
            end,
            { priority = -math.huge },
        },
        ['.*%.agda'] = 'agda',
        ['.*/.*%.als'] = 'alloy',
        ['.*%.ansible%.yaml'] = 'yaml.ansible',
        ['.*%.ansible%.yml'] = 'yaml.ansible',
        ['.*%.antlers%.html'] = 'html.antlers',
        ['.*/aura/.*/.*%.cmp'] = 'html',
        ['.*/aura/.*/.*%.css'] = 'css',
        ['.*/aura/.*/.*%.intf'] = 'xml',
        ['.*/aura/.*/.*%.js'] = 'javascript',
        ['.*/aura/.*/.*%.svg'] = 'svg',
        ['.*/aura/.*/.*%.tokens'] = 'xml',
        ['.*%.blade%.php'] = 'blade',
        ['.*%.cairo'] = 'cairo',
        ['.*%.ch%.hcl'] = 'atlas-schema-clickhouse',
        ['.*%.clar'] = 'clar',
        ['.*%.clarity'] = 'clarity',
        ['.*/classes/.*%.cls'] = 'apex',
        ['.*%.cls'] = 'apex',
        ['.*/cobol_src/.*'] = 'cobol',
        ['.*/components/.*%.component'] = 'html',
        ['Containerfile%..+'] = 'dockerfile',
        ['.*/content/.*%.md'] = 'markdown',
        ['.*/copybooks/.*'] = 'cobol',
        ['.*%.crystal'] = 'crystal',
        ['.*%.d%.ts'] = 'typescript',
        ['Dockerfile%..+'] = 'dockerfile',
        ['.*%.edge'] = 'edge',
        ['.*%.eex'] = 'html-eex',
        ['.*/ghostty/config'] = 'ghostty',
        ['ghostty%.conf'] = 'ghostty',
        ['^.*/gitconfig.*$'] = 'gitconfig',
        ['^.*/gitignore.*$'] = 'gitignore',
        ['^.*/gitcommit.*$'] = 'gitcommit',
        ['.*/%.github[%w/]+workflows[%w/]+.*%.yaml'] = 'yaml.github',
        ['.*/%.github[%w/]+workflows[%w/]+.*%.yml'] = 'yaml.github',
        ['%.gitlab%-ci%.yaml'] = 'yaml.gitlab',
        ['%.gitlab%-ci%.yml'] = 'yaml.gitlab',
        ['.*%.gjs$'] = 'javascript.glimmer',
        ['.*%.gts$'] = 'typescript.glimmer',
        ['%.hammerspoon/.*%.lua'] = 'lua',
        ['.*/handlers/.*%.yaml'] = 'yaml.ansible',
        ['.*/handlers/.*%.yml'] = 'yaml.ansible',
        ['.*%.heex'] = 'html-eex',
        ['.*%.html%.antlers'] = 'html.antlers',
        ['.*/hypr/.+%.conf'] = 'hyprlang',
        ['.*/hyprland%.conf'] = 'hyprlang',
        ['.*%.ipynb'] = 'json',
        ['.*%.lagda%.md'] = 'agda',
        ['.*%.lagda%.typ'] = 'agda',
        ['.*/layouts/.*%.html'] = 'html',
        ['.*%.lt%.hcl'] = 'atlas-schema-sqlite',
        ['.*%.lua'] = 'lua',
        ['.*/lua/.*%.lua'] = 'lua',
        ['.*/lwc/.*/.*%.css'] = 'css',
        ['.*/lwc/.*/.*%.html'] = 'html',
        ['.*/lwc/.*/.*%.js'] = 'javascript',
        ['.*/lwc/.*/.*%.js%-meta%.xml'] = 'xml',
        ['.*/lwc/.*/.*%.svg'] = 'svg',
        ['.*%-meta%.xml'] = 'xml',
        ['.*/[Mm]odules/.*/.*%.psm1'] = 'ps1',
        ['.*/[Mm]odules/.*/.*%.psd1'] = 'ps1',
        ['%.mount'] = 'systemd',
        ['.*%.ms%.hcl'] = 'atlas-schema-mssql',
        ['.*%.my%.hcl'] = 'atlas-schema-mysql',
        ['/nginx/.+%.conf'] = 'nginx',
        ['.*/pages/.*%.page'] = 'html',
        ['.*%.pg%.hcl'] = 'atlas-schema-postgresql',
        ['%.pg%.sql$'] = 'pgsql',
        ['.*%.plan%.hcl'] = 'atlas-plan',
        ['.*/playbooks/.*%.yaml'] = 'yaml.ansible',
        ['.*/playbooks/.*%.yml'] = 'yaml.ansible',
        ['.*[Pp]ower[Ss]hell/.*%.ps1'] = 'ps1',
        ['.*%.postcss'] = 'postcss',
        ['%.postgres%.sql$'] = 'pgsql',
        ['.*[Pp]rofile%.ps1$'] = 'ps1',
        ['.*%.razor'] = 'razor',
        ['.*/roles/.*%.yaml'] = 'yaml.ansible',
        ['.*/roles/.*%.yml'] = 'yaml.ansible',
        ['.*%.rs%.hcl'] = 'atlas-schema-redshift',
        ['.*%.rule%.hcl'] = 'atlas-rule',
        ['.*ScriptAnalyzer.*%.psd1$'] = 'ps1',
        ['%.service'] = 'systemd',
        ['%.socket'] = 'systemd',
        ['.*/[Ss]cripts/.*%.ps1'] = 'ps1',
        ['.*%.sugarss'] = 'sugarss',
        ['.*/tasks/.*%.yaml'] = 'yaml.ansible',
        ['.*/tasks/.*%.yml'] = 'yaml.ansible',
        ['.*/templates/.*%.yaml'] = 'helm',
        ['.*/templates/.*%.yml'] = 'helm',
        ['.*/templates/.*%.tpl'] = 'helm',
        ['.*%.test%.hcl'] = 'atlas-test',
        ['%.timer'] = 'systemd',
        ['.*%-tplg%.conf'] = 'alsaconf',
        ['.*%.tplg%.conf'] = 'alsaconf',
        ['.*%.trigger'] = 'apex',
        ['.*/triggers/.*%.trigger'] = 'apex',
        ['tsconfig.*%.json'] = 'jsonc',
        ['.*/values.*%.yaml'] = 'yaml.helm-values',
        ['.*/values.*%.yml'] = 'yaml.helm-values',
        ['.*/[Ww]indows[Pp]ower[Ss]hell/.*%.ps1'] = 'ps1',
        ['.*%.xaml'] = 'xaml',
        ['.*/%.zig%-cache/.*'] = 'zig',
        ['.*%.zon'] = 'zon',
    },
}

---Register once at require-time for compatibility, and allow explicit overrides later.
function M.setup(extra)
    assert(extra == nil or type(extra) == 'table', 'filetype options must be a table')
    vim.filetype.add(vim.tbl_deep_extend('force', vim.deepcopy(definitions), extra or {}))
end
M.setup()
return M
