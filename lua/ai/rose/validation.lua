-- /qompassai/Diver/lua/ai/rose/validation.lua
-- Host-owned verification gate for agent work (Tiger Style)
-- Copyright (C) 2025 Qompass AI, All rights reserved
-- ----------------------------------------
-- Verification is host-owned, not a model assertion: named checks, lint,
-- editor diagnostics and workspace freshness snapshots combine into a
-- verified/failed/unverified verdict. Cached diagnostics are advisory and
-- can never satisfy the gate alone.

local M = {}

---@param config table resolved Rose config
---@return table<string, table> snapshots keyed by buffer handle
local function workspace_buffers(config)
    local snapshots = {}
    for _, buffer in ipairs(vim.api.nvim_list_bufs()) do
        local name = vim.api.nvim_buf_get_name(buffer)
        if vim.api.nvim_buf_is_loaded(buffer) and name:sub(1, #config.workspace + 1) == config.workspace .. '/' then
            snapshots[buffer] = {
                name = name,
...
---@param config table
---@param report table validation report carrying buffer_snapshots
---@param tools table host tool module
---@param context_buf integer?
---@return boolean fresh
function M.is_fresh(config, report, tools, context_buf)
    assert(type(config) == 'table', 'validation.is_fresh: config must be a table')
    assert(type(report) == 'table', 'validation.is_fresh: report must be a table')
    local current = workspace_buffers(config)
    for buffer, snapshot in pairs(report.buffer_snapshots or {}) do
        local now = current[buffer]
        if not now or now.name ~= snapshot.name or now.changedtick ~= snapshot.changedtick or now.modified then
            return false
        end
    end
...
-- Call a host tool without letting a tool exception escape; every failure
-- becomes a { status = 'error' } result so the gate can record it.
---@param tools table host tool module
---@param name string
---@param args table?
---@param context_buf integer?
...
function M.safe_call(tools, name, args, context_buf)
    assert(type(tools) == 'table', 'validation.safe_call: tools must be a table')
    assert(type(name) == 'string', 'validation.safe_call: name must be a string')
...
local function is_error_diagnostic(diagnostic)
    return diagnostic.severity == 1 or diagnostic.severity == 'ERROR' or diagnostic.severity == 'error'
end

---@param config table
---@param name string?
---@return string[] names  -- sorted; explicit name wins
...
---@param changed_paths string[]?
---@return (string|false)[] targets
...
---@param report table
---@param verdict table
---@param job_count integer
local function evaluate_checks(report, verdict, job_count)
    if job_count == 0 then
        report.reasons[#report.reasons + 1] = 'No applicable named checks configured; validation is unverified.'
    end
...
local function conclude(run)
    ...
    local context = M.safe_call(run.tools, 'editor_context', { path = targets[1] or nil }, run.context_buf)
...
---@param config table
---@param tools table host tool module
---@param name string? run only this named check
...
function M.run(config, tools, name, callback, context_buf, changed_paths)
    ...
    local function check_at(index)
        ...
        local result = M.safe_call(tools, 'editor_check', { name = job.name, path = job.path }, context_buf)
    ...
    local function lint_at(index)
        ...
        report.lint[index] = {
            path = path,
            result = M.safe_call(tools, 'editor_lint', { path = path }, context_buf),
        }
        vim.schedule(function()
            lint_at(index + 1)
        end)
