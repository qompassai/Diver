-- /qompassai/Diver/lua/ai/rose/provider/nvidia.lua
-- Nvidia legacy provider class (Tiger Style)
...
function Nvidia:get_available_models(online)
    ...
    if online and self:verify() then
        ...
        if type(key) ~= 'string' then
            logger.error('NVIDIA - API key unavailable after verification')
            return ids
        end
        local stdout = curl_get('https://integrate.api.nvidia.com/v1/models', key)
