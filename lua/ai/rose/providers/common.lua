-- /qompassai/Diver/lua/ai/rose/providers/common.lua
-- Shared normalized-chat helpers for cloud provider adapters (Tiger Style)
...
function M.fields(value, allowed, kind)
    ...
    for key in pairs(value) do
        assert(allowed[key], 'unsupported ' .. kind .. ' field; use the raw JSON request API for native fields')
    end
end
...
function M.options(config, payload)
    ...
    for key, value in pairs(config.options or {}) do
        assert(
            ...
            'provider options cannot override model/messages/input/tools/stream; use the raw JSON ' .. 'request API'
        )
        payload[key] = M.copy(value)
    end
    assert(payload.n == nil or payload.n == 1, 'normalized chat supports n=1 only; use the raw JSON request API')
    return payload
end
...
function M.messages(messages, config)
    ...
    for _, message in ipairs(messages) do
        ...
        if message.role == 'tool' then
            ...
            assert(pending[message.tool_call_id], 'tool result id does not match a pending assistant call')
            pending[message.tool_call_id] = nil
        else
            assert(next(pending) == nil, 'all assistant tool calls need results before another conversation message')
        end
    ...
    assert(next(pending) == nil, 'all assistant tool calls require matching results before the next model request')
end

---@param calls table[]
---@param native boolean? skip normalized-shape checks for provider-native calls
...
function M.calls(calls, native)
    ...
    for _, call in ipairs(calls) do
        ...
        assert(call.type == nil or call.type == 'function', 'unsupported tool call; use the raw JSON request API')
        local fn = call['function']
        assert(type(fn) == 'table' and type(fn.name) == 'string' and fn.name ~= '', 'invalid provider tool function')
        if not native then
            M.fields(fn, { name = true, arguments = true }, 'normalized tool function')
        end
        assert(type(fn.arguments) == 'string' or type(fn.arguments) == 'table', 'invalid provider tool arguments')
        ids[call.id] = true
        result[#result + 1] = {
            id = call.id,
            type = 'function',
...
---@param tools table[]?
---@param format string 'chat' | 'messages' | 'responses'
---@param config table?
---@return table[] wire_tools
...
---@param text string?
---@return string public_text
function M.public_text(text)
    assert(not M.present(text) or type(text) == 'string', 'unsupported assistant content; use raw JSON request API')
...
---@param config table
---@param response table raw provider response
---@param text string?
---@param calls table[]?
---@param reason string?
---@param citations table[]?
---@return table message normalized assistant message
