-- /qompassai/Diver/lua/ai/rose/providers/anthropic.lua
-- Anthropic Messages API encode/decode (Tiger Style)
-- Copyright (C) 2025 Qompass AI, All rights reserved
-- ----------------------------------------
-- Normalized chat over the Anthropic Messages API. Reasoning and
-- thinking-block content is never surfaced as text.

local C = require('ai.rose.providers.common')
local M = {}

---@param config table resolved provider config
...
---@param config table resolved provider config
---@param response table decoded provider response
...
function M.decode(config, response)
    ...
    for _, block in ipairs(response.content) do
        ...
        then
            -- Provider-owned blocks (thinking, redacted_thinking, server_tool_use,
            -- *_tool_result) are retained verbatim in _provider.response.content.
            error('unsupported Anthropic content block; use the raw JSON request API', 0)
        end
    end
    return C.result(config, response, table.concat(text, '\n'), calls, response.stop_reason, citations)
end
