-- /qompassai/Diver/lua/ai/rose/providers/sse.lua
-- Incremental SSE parser for provider streams (Tiger Style)
-- Copyright (C) 2025 Qompass AI, All rights reserved
-- ----------------------------------------
-- A raw event API, never wired directly into the text UI.
...
local function decode_item(event, id, text)
    ...
    item.data = value
    local api_error = event == 'error' or value.type == 'error' or value.error or value.type == 'response.failed'
    if api_error then
...
---@class AiRoseSseParser
---@field feed fun(chunk: string)
---@field finish fun(): table?, string?
---@field error fun(): string?

---@param on_event fun(item: table)? raw event consumer
---@param max_event_bytes integer?
---@return AiRoseSseParser parser
