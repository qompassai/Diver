-- /qompassai/Diver/lua/ai/rose/providers/init.lua
-- Lazy, explicit cloud-provider registry (Tiger Style)
-- Copyright (C) 2025 Qompass AI, All rights reserved
-- ----------------------------------------
-- Requiring/validating never reads keys. The key is read from the
-- environment only at request time, and cloud egress needs both
-- providers.enabled and providers.allow_cloud.

local C = require('ai.rose.providers.common')
local T = require('ai.rose.providers.transport')
local M = {}
...
local function resolve_endpoint(config, descriptor, name)
    ...
    if config.credential_host ~= nil then
        assert(config.credential_host == endpoint.authority, 'credential_host must exactly match endpoint host[:port]')
    else
        assert(
    ...
    assert(
        ...
        'provider endpoint requires HTTPS; HTTP is permitted only for explicitly allowed ' .. 'loopback NIM/fixtures'
    )
...
local function resolve_limits(config)
    ...
    config.max_event_bytes = config.max_event_bytes or math.min(1024 * 1024, config.max_response_bytes)
...
-- Resolve the configured cloud provider; returns the provider config and its API descriptor.
---@param fullconfig table resolved Rose config
---@return table config
---@return table api
function M.resolve(fullconfig)
    ...
    if name ~= 'anthropic' then
        descriptor = require('ai.rose.providers.' .. name)
    end
...
local function request_spec_valid(spec)
    return pcall(function()
        C.fields(spec, { path = true, method = true, body = true, stream = true, on_event = true }, 'JSON request')
...
---@class AiRoseRawRequestSpec
---@field path string
---@field method string?
---@field body table?
---@field stream boolean?
---@field on_event fun(item: table)?

-- Raw JSON request against the resolved provider endpoint.
---@param fullconfig table resolved Rose config
---@param spec AiRoseRawRequestSpec
---@param callback fun(err: string?, result: table?)
---@return table token
function M.request(fullconfig, spec, callback)
    ...
    if not request_spec_valid(spec) then
        return rejected(callback, 'invalid raw JSON request (check path, body, stream and callback options)')
    end
    local headers, header_err = request_headers(config)
    if not headers then
        return rejected(callback, header_err or 'invalid request headers')
    end
    local body
    if spec.body then
    ...
    if spec.stream then
        parser = require('ai.rose.providers.sse').new(function(event)
...
-- Normalized chat: encode messages, POST to the API path, decode the assistant message.
---@param fullconfig table resolved Rose config
---@param messages table[] normalized messages
---@param tools table[]? function tool schemas
...
function M.chat(fullconfig, messages, tools, callback)
    ...
    if tools and #tools > 0 and (not api.tools or config.capabilities.tools ~= true) then
        return rejected(
            callback,
            'this API/model has not enabled custom tools; explicitly set '
                .. 'capabilities.tools=true on '
                .. 'a supported model (Sonar is not supported)'
        )
    end
    local adapter = require('ai.rose.providers.' .. api.format)
        ...
        if not decoded then
            ...
            callback(
                'invalid or unsupported '
                    .. config.name
                    .. ' assistant response; use raw JSON request API for '
                    .. 'non-text/non-function features'
            )
            return
        end
