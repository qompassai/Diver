-- /qompassai/Diver/lua/ai/rose/providers/responses.lua
-- Responses-style API encode/decode (Tiger Style)
...
function M.decode(config, response)
    assert(type(response.output) == 'table' and vim.islist(response.output), 'provider response has no output array')
    assert(
    ...
    local reason = response.incomplete_details and response.incomplete_details.reason or response.status
