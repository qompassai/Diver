...
local function to_module(root, path)
  ...
  rel = rel:gsub('%.lua$', '')
  rel = rel:gsub('/', '.')
  if rel:match('%.init$') then rel = rel:gsub('%.init$', '') end
  return rel
end
...
local function selfcheck()
  ...
  for _, file in ipairs(files) do
    ...
    if ok then
      ok_count = ok_count + 1
      if fh then fh:write(('[selfcheck] OK: %s\n'):format(mod)) end
    else
  ...
  vim.notify(summary, err_count == 0 and vim.log.levels.INFO or vim.log.levels.ERROR)
  if fh then
