...
local function collect_results(value, results, depth)
  ...
  for _, child in pairs(value) do
    if type(child) == 'table' then
      collect_results(child, results, depth + 1)
    end
  end
end
...
local function warning_code(warning)
  return string_value(warning.rule) or string_value(warning.code)
end

---@param warning table
---@return string
local function warning_message(warning)
  local message = string_value(warning.text) or string_value(warning.message) or 'Stylelint diagnostic'
...
local function warning_diagnostic(warning, context)
  ...
  if warning.endLine ~= nil then
    end_lnum = zero_based(warning.endLine)
  end

  if warning.endColumn ~= nil then
    end_col = zero_based(warning.endColumn)
  end
  ...
  return {
    ...
    severity = severity(string_value(warning.severity)),
    ...
    user_data = {
      ...
      severity = string_value(warning.severity),
    },
  }
end
...
local function parse_result(result, context, diagnostics)
  ...
  for _, warning in ipairs(result.warnings) do
    ...
    if type(warning) == 'table' then
      diagnostics[#diagnostics + 1] = warning_diagnostic(warning, context)
    end
  end
end
  ...
  return {
    {
      ...
      message = string.format('Stylelint output exceeded the %d-byte parser limit', MAX_OUTPUT_BYTES),
...
local function parse(output, context)
  assert(type(context) == 'table', 'stylelint parser requires LintContext')

  assert(type(context.bufnr) == 'number', 'stylelint parser requires context.bufnr')
  ...
  collect_results(decoded, results, 0)
  ...
  for index = 1, #results do
    parse_result(results[index], context, diagnostics)
...
local function arguments(context)
  assert(type(context) == 'table', 'stylelint arguments require LintContext')
