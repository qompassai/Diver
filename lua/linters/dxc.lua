...
local function infer_profile(filename)
    local configured = string_value(vim.env.NVIM_DXC_PROFILE)
    ...
    for suffix, profile in pairs(PROFILE_SUFFIXES) do
...
local function entry_point()
    return string_value(vim.env.NVIM_DXC_ENTRY) or DEFAULT_ENTRY
end

---@return string
local function hlsl_version()
    return string_value(vim.env.NVIM_DXC_HLSL_VERSION) or DEFAULT_HLSL_VERSION
end
...
local function severity(level)
    ...
    if normalized == 'note' or normalized == 'info' then
...
local function parse_finding(line)
    ...
    path, line_number, column, level, message, code =
        line:match('^(.+):(%d+):(%d+):%s+(fatal error|error|warning|note):%s+(.+)%s+%[([^%]]+)%]$')

    if path ~= nil and line_number ~= nil and column ~= nil and level ~= nil and message ~= nil then
        return {
            code = code,

            column = math.floor(tonumber(column) or 1),
    ...
    path, line_number, column, level, message =
        line:match('^(.+):(%d+):(%d+):%s+(fatal error|error|warning|note):%s+(.+)$')
    ...
    --
    path, line_number, level, message = line:match('^(.+):(%d+):%s+(fatal error|error|warning|note):%s+(.+)$')

    if path == nil or line_number == nil or level == nil or message == nil then
        return nil
    end
...
local function finding_diagnostic(finding, context)
    ...
    if finding.path ~= nil and finding.path ~= '<stdin>' and not same_file(finding.path, context) then
    ...
    return {
        {
            ...
            message = string.format('DXC output exceeded the %d-byte parser limit', MAX_OUTPUT_BYTES),
...
local function parse(output, context)
    assert(type(context) == 'table', 'dxc parser requires LintContext')

    assert(type(context.bufnr) == 'number', 'dxc parser requires context.bufnr')
...
local function arguments(context)
    assert(type(context) == 'table', 'dxc arguments require LintContext')
    ...
    if not library_profile(target) then
        table.insert(args, #args, entry_point())
    ...
    if truthy(vim.env.NVIM_DXC_SPIRV) then
        ...
        local environment = string_value(vim.env.NVIM_DXC_SPIRV_ENV)

        if environment ~= nil then
            table.insert(args, 2, '-fspv-target-env=' .. environment)
        end
    end
