...
local function severity(value)
    ...
    return SEVERITY[value:lower()] or diagnostic.severity.ERROR
end

---@param message string
---@return string?, string
local function warning_code(message)
    local code = message:match('%[%-(W[%w%-]+)%]%s*$')

    if code == nil then
        return nil, message
    end

    local cleaned = message:gsub('%s*%[%-(W[%w%-]+)%]%s*$', '')
...
local function belongs_to_buffer(path, context)
    ...
    return fs.basename(normalized_path) == fs.basename(normalized_filename)
end
...
local function make_diagnostic(message, level, line, column, code, context)
...
local function parse_located(line, context)
    ...
    if path ~= nil then
        ...
        return make_diagnostic(message, level, line_number, column, code, context)
    end

    path, line_number, column, message = line:match('^(.+):(%d+):(%d+):%s*(.+)$')
    ...
    if path ~= nil then
        ...
        return make_diagnostic(message, nil, line_number, column, code, context)
    end
    ...
    if path ~= nil then
        ...
        return make_diagnostic(message, level, line_number, 1, code, context)
    end
    ...
    return make_diagnostic(message, nil, line_number, 1, code, context)
end
...
local function parse_global(line, context)
    ...
    level, message = line:match('^ghdl:%s*([%a]+):%s*(.+)$')

    if level ~= nil then
        return make_diagnostic(message, level, 1, 1, nil, context)
    end

    level, message = line:match('^([%a]+):%s*(.+)$')

    if level ~= nil and SEVERITY[level:lower()] ~= nil then
        return make_diagnostic(message, level, 1, 1, nil, context)
    end
...
local function parse(output, context)
    assert(type(context) == 'table', 'ghdl parser requires LintContext')

    assert(type(context.bufnr) == 'number', 'ghdl parser requires context.bufnr')
    ...
    for raw_line in output:gmatch('[^\r\n]+') do
        ...
        if line ~= '' then
            local item = parse_located(line, context)

            if item == nil then
                item = parse_global(line, context)
            end

            if item ~= nil then
                diagnostics[#diagnostics + 1] = item
                previous = item
            elseif previous ~= nil and not line:match('^%^') and not line:match('^%s*|') then
                local continuation = compact(line)

                if continuation ~= '' then
                    previous.message = truncate(previous.message .. ' ' .. continuation, MAX_MESSAGE_BYTES)
                end
            end
        end
    end
...
local function args(context)
    assert(type(context) == 'table', 'ghdl args requires LintContext')

    local standard = nonempty_string(env.GHDL_STD) or '08'
