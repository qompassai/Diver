...
local function parse(output, context)
  ...
  if foreign then
    result[#result + 1] =
      status(context, 'other-files', 'Psalm reported issues outside this buffer; run project analysis to inspect them.')
  end
  if limited then
    result[#result + 1] =
      status(context, 'result-limit', 'Psalm reached the editor result limit; run project analysis for all findings.')
  end
  return result
end
