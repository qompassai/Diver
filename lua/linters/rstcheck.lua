...
local function parse_line(line)
    ...
    path, line_number, level, message = line:match('^(.+):(%d+):%s*%(([A-Z]+)/%d+%)%s+(.+)$')
    ...
    return path, integer(line_number, 1), severity, message
end
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'rstcheck parser requires a LintContext')
    ...
    for line in output:gmatch('[^\r\n]+') do
        ...
        if #line <= LINE_LENGTH_MAX then
            ...
            path, line_number, level, message = parse_line(line)
