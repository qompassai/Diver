...
---@source https://htmlhint.com/configuration/
...
local function baseline(root)
    ...
    local path = fs.joinpath(root, '.secrets.baseline')
...
---@param finding HtmlhintFinding
---@param filename string
---@param root string
---@return vim.Diagnostic.Set?
local function diagnostic_from_finding(finding, filename, root)
    ...
    local row = math.max(integer(finding.line_number, 1) - 1, 0)
...
---@return integer
local function append_findings(value, filename, root, diagnostics, diagnostics_count)
    assert(filename ~= '')
    assert(root ~= '')
    assert(diagnostics_count >= 0)
    assert(diagnostics_count <= DIAGNOSTICS_MAX)

    local count = math.min(#value, RESULTS_PER_FILE_MAX)
    ...
    for index = 1, count do
        ...
        if type(finding) == 'table' then
            ---@cast finding HtmlhintFinding

            local entry = diagnostic_from_finding(finding, filename, root)
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'detect-secrets parser requires a LintContext')
    ...
    assert(#output <= OUTPUT_LENGTH_MAX, 'detect-secrets output exceeded maximum size')
    ...
    --
    if #decoded > 0 then
        diagnostics_count = append_findings(decoded, filename, root, diagnostics, diagnostics_count)
    else
        for _, findings in pairs(decoded) do
            ...
            if type(findings) == 'table' then
                diagnostics_count = append_findings(findings, filename, root, diagnostics, diagnostics_count)
            end
        end
    end
...
return ---@type Linter
{
    ...
    args = function(context)
        ...
        local baseline_path = baseline(context.root)
