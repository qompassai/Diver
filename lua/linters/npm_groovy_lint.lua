...
---@field transform_buffer fun(bufnr: integer?, mode: NpmGroovyLintMode?): boolean, string?
---@field setup fun(options: NpmGroovyLintSetupOptions?)
...
local M = {
    ...
    -- Declared here, defined below via `function M.x` (LuaLS checks the
    -- literal, so the fields must be present with their final types).
    ---@type fun(options: NpmGroovyLintSetupOptions?)
    setup = nil,
    ---@type fun(bufnr: integer?, mode: NpmGroovyLintMode?): boolean, string?
    transform_buffer = nil,
}
...
local function write_temporary(filename, content)
    ...
    if descriptor == nil then
        ...
        return nil,
            with_cleanup_problem('cannot create private formatter input: ' .. tostring(open_error), cleanup_problem)
    end
