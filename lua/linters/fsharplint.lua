...
local function integer(value)
  return type(value) == 'number' and value == value and value >= 1 and value < 2147483647 and value == math.floor(value)
end
...
local function parse(output, context)
  ...
  local function position(row, column)
    ...
    if not integer(row) or row > count or not column or column < 0 or column ~= math.floor(column) then
      return nil
    end
    local lnum = row - 1
