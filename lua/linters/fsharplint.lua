...
local function config_file(context)
    ...
    if override then
        return absolute(override, root(context))
    end
...
local function parse(output, context)
    ...
    local function position(row, column)
        ...
        -- The explicit nil checks narrow the types for the checker; integer()
        -- alone cannot narrow, but it safely rejects nil at runtime.
        if row == nil or not integer(row) or row > count then
            return nil
        end
        if not column or column < 0 or column ~= math.floor(column) then
            return nil
        end
        local lnum = math.floor(row) - 1
    ...
    for raw in output:gmatch('[^\r\n]+') do
        ...
        if records > MAX_RECORDS or #diagnostics >= MAX_DIAGNOSTICS then
            diagnostics[#diagnostics + 1] =
                status(context, 'diagnostic-limit', 'FSharpLint report was truncated by the parser limit.')
            break
        end
        local line = raw:gsub('\27%[[%d;]*[mK]', ''):gsub('\r$', '')
        local path, sl, sc, el, ec, rule, message =
            line:match('^(.*)%((%d+),(%d+),(%d+),(%d+)%)%s*:%s*FSharpLint warning%s+(FL%d+):%s*(.*)$')
        if path then
        ...
        elseif
            line:match('^Running FSharpLint')
            or line:match('^Starting')
            or line:match('^WARNING: Using a project')
            or line:match('^%-+$')
            or line:match('^=+ Linting ')
            or line:match('^=+ Finished: ')
        then
            last = nil
        elseif vim.trim(line) ~= '' then
            if last then
                last.message = clean(last.message .. ' ' .. line)
            elseif not unrecognized then
                unrecognized = clean(line)
            end
        end
    end
    if not saw_warning and not saw_completion and #diagnostics == 0 then
        diagnostics[1] = status(
            context,
            'no-report',
            'FSharpLint did not return a recognized report. '
