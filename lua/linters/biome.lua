...
local function executable(root)
    ...
    local local_cmd = fs.joinpath(root, 'node_modules', '.bin', 'biome')
...
local function parse_line(line)
    ...
    symbol, path, line_number, column_number, code, message =
        line:match('^%s*([×!i])%s+(.+):(%d+):(%d+):%s+([^:]+):%s+(.+)$')
    ...
    return path, integer(line_number, 1), integer(column_number, 1), code, message, level
end
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'biome parser requires a LintContext')
    ...
    for line in output:gmatch('[^\r\n]+') do
        ...
        if #line <= LINE_LENGTH_MAX then
            ...
            path, line_number, column_number, code, message, level = parse_line(line)
