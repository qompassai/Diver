...
local function absolute_path(path, base)
    ...
    return fs.normalize(fs.joinpath(base, path))
end
...
local function decode_record(value)
    ...
    local message = string_value(value.message)
...
local function record_diagnostic(record, context)
    ...
    local path = string_value(location.file)

    if path == nil then
        return nil
    end

    local package_root = package_directory(context)

    local absolute = absolute_path(path, package_root)

    if not nonempty_string(context.filename) or not same_path(absolute, fs.normalize(context.filename)) then
        return nil
    end

    local lnum = zero_based(location.line)

    local col = zero_based(location.column)

    local end_lnum = lnum
    local end_col = col

    local record_end = record['end']

    if type(record_end) == 'table' then
        local finish = record_end

        if finish.line ~= nil then
            end_lnum = zero_based(finish.line)
        end

        if finish.column ~= nil then
            end_col = zero_based(finish.column)
        end
    end
    ...
    local message = string_value(record.message)
    ...
    return {
        ...
        user_data = {
            check = string_value(record.code),
...
local function operational_message(text)
    ...
    for raw_line in cleaned:gmatch('[^\r\n]+') do
        ...
        if
            ...
            or lower:find('compile', 1, true) ~= nil
...
---@param context LintContext
---@return vim.Diagnostic
local function operational_diagnostic(message, context)
    return {
        bufnr = context.bufnr,
    ...
    return {
        {
            ...
            message = string.format('Staticcheck output exceeded the %d-byte parser limit', MAX_OUTPUT_BYTES),
...
local function parse(output, context)
    assert(type(context) == 'table', 'staticcheck parser requires LintContext')

    assert(type(context.bufnr) == 'number', 'staticcheck parser requires context.bufnr')
    ...
    for raw_line in text:gmatch('[^\r\n]+') do
        ...
        if line ~= '' then
            local record = parse_json_line(line)

            if record ~= nil then
                if record.severity ~= 'ignored' then
                    local item = record_diagnostic(record, context)
    ...
    if #diagnostics == 0 and #unparsed > 0 then
        local message = operational_message(table.concat(unparsed, '\n'))

        if message ~= nil then
            diagnostics[1] = operational_diagnostic(message, context)
        end
    end
...
local function arguments(context)
    assert(type(context) == 'table', 'staticcheck arguments require LintContext')
...
local function cwd(context)
    return package_directory(context)
end
