...
local function parse(output, context)
  ...
  assert(type(context) == 'table', 'hadolint parser requires a LintContext')

  ---@cast context LintContext

  assert(context.filename ~= '')
  assert(context.root ~= '')
  assert(#output <= OUTPUT_LENGTH_MAX, 'hadolint output exceeded maximum size')
  ...
  local violations_count = math.min(#decoded, DIAGNOSTICS_MAX)
...
local function args(context)
  ...
  append_repeated(argv, '--error', error_rules)

  append_repeated(argv, '--warning', warning_rules)

  append_repeated(argv, '--require-label', required_labels)

  append_repeated(argv, '--trusted-registry', trusted_registries)
