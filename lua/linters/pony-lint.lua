...
local function project_root(context)
    ...
    if nonempty_string(context.filename) then
        ...
        --
        local corral_root = fs.root(context.filename, {
            'corral.json',
        })

        if type(corral_root) == 'string' and corral_root ~= '' then
            return normalize_path(corral_root)
        end

        local git_root = fs.root(context.filename, {
            '.git',
        })
        ...
        local lint_root = fs.root(context.filename, {
            '.pony-lint.json',
        })

        if type(lint_root) == 'string' and lint_root ~= '' then
            return normalize_path(lint_root)
        end
    ...
    if nonempty_string(context.cwd) then
        return normalize_path(context.cwd)
    end
...
local function severity(rule)
    ...
    if rule:find('safety/', 1, true) == 1 then
        return diagnostic.severity.ERROR
    end

    if rule:find('lint/', 1, true) == 1 then
        return diagnostic.severity.ERROR
    end

    if rule:find('style/', 1, true) == 1 then
        return diagnostic.severity.WARN
    end
...
local function normalize_rule(value)
    if value == nil or not nonempty_string(value) then
        return nil
    end
    ...
    if rule:match('^[%a][%w_-]*/[%w_-]+$') ~= nil then
        return rule
    end
...
local function parse_record(line)
    ...
    -- message rendering.
    --
    path, line_number, column, rest = line:match('^(.+):(%d+):(%d+):%s*(.+)$')

    if path == nil or line_number == nil or column == nil or rest == nil then
        return nil
    end
    ...
    --
    rule, message = rest:match('^([%a][%w_-]*/[%w_-]+):%s*(.+)$')

    if rule ~= nil and message ~= nil then
        return {
            column = column,
    ...
    --
    message, rule = rest:match('^(.-)%s+%[([%a][%w_-]*/[%w_-]+)%]%s*$')
...
local function record_diagnostic(record, context)
    local rule = normalize_rule(record.rule)

    local lnum = zero_based(record.line)

    local col = zero_based_column(record.column)

    local message = compact(record.message)

    local record_path = record.path or ''

    if record_path ~= '' and context.filename ~= '' and not same_path(record_path, context.filename) then
        message = string.format('%s: %s', record_path, message)
...
local function looks_operational(line)
    ...
    return lower:find('error', 1, true) ~= nil
        or lower:find('unreadable', 1, true) ~= nil
        or lower:find('malformed', 1, true) ~= nil
    ...
    return {
        {
            ...
            message = string.format('pony-lint output exceeded the %d-byte parser limit', MAX_OUTPUT_BYTES),
...
local function parse(output, context)
    assert(type(context) == 'table', 'pony-lint parser requires LintContext')

    assert(type(context.bufnr) == 'number', 'pony-lint parser requires context.bufnr')
...
local function arguments(context)
    assert(type(context) == 'table', 'pony-lint arguments require LintContext')
