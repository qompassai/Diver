...
local function diagnostic_from_offense(offense, path, filename, root)
    ...
    local start_line = math.max(integer(location.start_line or location.line, 1) - 1, 0)

    local start_column = math.max(integer(location.start_column or location.column, 1) - 1, 0)

    local end_line = math.max(integer(location.last_line, start_line + 1) - 1, start_line)

    local minimum_end_column = end_line == start_line and start_column + 1 or 0

    local end_column = math.max(integer(location.last_column, minimum_end_column + 1), minimum_end_column + 1)
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'rubocop parser requires a LintContext')
    ...
    assert(#output <= OUTPUT_LENGTH_MAX, 'rubocop output exceeded maximum size')
    ...
    for file_index = 1, #files do
        ...
        if type(file) == 'table' then
            ...
            if type(path) == 'string' and path ~= '' and type(offenses) == 'table' then
                local offense_count = math.min(#offenses, DIAGNOSTICS_MAX - diagnostics_count)

                for offense_index = 1, offense_count do
                    local offense = offenses[offense_index]

                    if type(offense) == 'table' then
                        local entry = diagnostic_from_offense(offense, path, filename, root)
