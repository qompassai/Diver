...
local function system_unit_path(filename)
  ...
  for directory in pairs(SYSTEMD_DIRECTORIES) do
    if normalized:find('/' .. directory .. '/', 1, true) ~= nil then
      return true
    end
  end
...
local function parse_line(line)
  ...
  --
  local path, line_number, level, code, message = line:match('^(.-):(%d+):([^%s]+)%s+%[([^%]]+)%]%s+%-%s+(.+)$')

  if path == nil or line_number == nil or level == nil or code == nil or message == nil then
    return nil
  end
  ...
  return {
    {
      ...
      message = string.format('systemdlint output exceeded the %d-byte parser limit', MAX_OUTPUT_BYTES),
...
local function parse(output, context)
  assert(type(context) == 'table', 'systemdlint parser requires LintContext')

  assert(type(context.bufnr) == 'number', 'systemdlint parser requires context.bufnr')
  ...
  for line in text:gmatch('[^\r\n]+') do
    ...
    local finding = parse_line(vim.trim(line))
...
local function arguments(context)
  assert(type(context) == 'table', 'systemdlint arguments require LintContext')
