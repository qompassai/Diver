...
local function current_source(source, context)
    ...
    return canonical(source, root(context)) == canonical(context.filename, nonempty(context.cwd) or vim.fn.getcwd())
end
...
local function json_path(value)
    ...
    for index = 1, math.min(#value, 128) do
        ...
        if type(part) == 'string' then
            parts[#parts + 1] = clean(part)
        elseif type(part) == 'number' and integer(part) then
            parts[#parts + 1] = part
        end
    end
    return parts
...
local function parse(output, context)
    ...
    if #output > MAX_OUTPUT then
        return {
            status(context, 'output-limit', 'Spectral output exceeded the 16 MiB parser limit.'),
        }
    end
    ...
    local function position(value)
        if type(value) ~= 'table' or not integer(value.line) or not integer(value.character) or value.line >= count then
            return nil
        end
    ...
    for index = 1, math.min(#report, MAX_RECORDS) do
        if #diagnostics >= MAX_DIAGNOSTICS then
            diagnostics[#diagnostics + 1] =
                status(context, 'diagnostic-limit', 'Only the first 512 Spectral diagnostics are shown.')
            break
        end
        ...
        elseif finding.severity ~= -1 then
            ...
            if source == nil or current_source(source, context) then
                ...
                if type(finding.code) == 'number' and integer(finding.code) then
    ...
    if malformed then
        diagnostics[#diagnostics + 1] =
            status(context, 'invalid-record', 'Some Spectral report records were malformed.')
    end
    if #report > MAX_RECORDS then
        diagnostics[#diagnostics + 1] =
            status(context, 'record-limit', 'Spectral report exceeded the 10000-record parser limit.')
    end
    return diagnostics
end
