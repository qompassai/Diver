--- gawk linter adapter — awk script warning checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `gawk` program and turn its complaints into squiggles under your code. It only runs when
--- linting is triggered (usually on save), and only if `gawk` is installed on your computer.
---@module 'linters.gawk'
-- #################################################################
...
local function parse(output, context)
    ...
    for line in output:gmatch('[^\r\n]+') do
        ...
        -- 'In file included from' lines are skipped; the following finding
        -- carries the included filename and line.
        if not line:match('^.-gawk: In file included from ') and file and row and message then
            ...
            if caret or explicit then
                ...
                local item = finding(
                    context,
                    text,
                    file == '-' and math.floor(tonumber(row) or 1) or nil,
