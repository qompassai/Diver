...
local function parse_position(position)
    ...
    path, line_number, column_number = position:match('^(.-):(%d+):(%d+)$')

    if path == nil then
        return nil
    end

    return path, integer(line_number, 1), integer(column_number, 1)
end
...
local function diagnostic_from_entry(entry, filename, root)
    ...
    if type(position) ~= 'string' or position == '' or type(message) ~= 'string' or message == '' then
        return nil
    end
    ...
    path, line_number, column_number = parse_position(position)

    if path == nil or line_number == nil or column_number == nil then
        return nil
    end
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'fieldalignment parser requires a LintContext')
    ...
    assert(#output <= OUTPUT_LENGTH_MAX, 'fieldalignment output exceeded maximum size')
    ...
    for _, package_result in pairs(decoded) do
        ...
        if type(package_result) == 'table' then
            ...
            if type(entries) == 'table' then
                local entries_count = math.min(#entries, DIAGNOSTICS_PER_PACKAGE_MAX)
                ...
                for index = 1, entries_count do
                    ...
                    if type(entry) == 'table' then
                        local item = diagnostic_from_entry(entry, filename, root)

                        if item ~= nil then
                            diagnostics_count = diagnostics_count + 1

                            diagnostics[diagnostics_count] = item
                        end
                    end
                end
            end
...
return ---@type Linter
{
    ...
    args = function(context)
        ...
        return {
            '-json',
            package_pattern(context.filename, context.root),
        }
    end,
    ...
    cwd = function(context)
        ...
        --
        return fs.dirname(fs.normalize(context.filename))
    end,
