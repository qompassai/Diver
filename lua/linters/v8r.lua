--- v8r linter adapter — JSON/YAML validity checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `v8r` program and turn its complaints into squiggles under your code. It only runs when
--- linting is triggered (usually on save), and only if `v8r` is installed on your computer.
---@module 'linters.v8r'
-- #################################################################
    ...
    if not ok or type(decoded) ~= 'table' or type(decoded.results) ~= 'table' or not vim.islist(decoded.results) then
        return {
            status_diagnostic(
                context,
                'invalid-report',
                'v8r returned no valid JSON results. Check configuration, ignored files, '
                    .. 'CLI compatibility and the timeout.'
            ),
        }
    end
        ...
        elseif canonical_path(result.fileLocation, cwd) == target then
                    ...
                    if type(issue) ~= 'table' or string_value(issue.message) == nil then
                        ...
                        diagnostics[#diagnostics + 1] = {
                            ...
                            user_data = {
                                ...
                                schema_path = type(issue.schemaPath) == 'string' and clean_message(issue.schemaPath)
                                    or nil,
            ...
            elseif result.valid ~= true or result.code ~= 0 then
                diagnostics[#diagnostics + 1] = status_diagnostic(
                    ...
                    prefix
                        .. 'v8r could not complete validation. Check input syntax, '
                        .. 'schema discovery, schema references and network access.'
                )
            end
