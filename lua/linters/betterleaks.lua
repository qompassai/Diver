...
local function diagnostic_from_result(result, filename, root)
    ...
    local message = type(message_table) == 'table' and message_table.text or nil
    ...
    local end_line = math.max(integer(region.endLine, start_line + 1) - 1, start_line)

    local end_column =
        math.max(integer(region.endColumn, start_column + 2) - 1, end_line == start_line and start_column + 1 or 0)
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'betterleaks parser requires a LintContext')
    ...
    for run_index = 1, runs_count do
        ...
        if type(results) == 'table' then
            ...
            for result_index = 1, results_count do
                ...
                local entry = diagnostic_from_result(results[result_index], filename, root)
