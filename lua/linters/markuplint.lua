...
local function parse(output, context)
  ...
  if not ok or text:sub(1, 1) ~= '[' or type(report) ~= 'table' or not vim.islist(report) then
    return {
      status(
        context,
        'invalid-report',
        'markuplint did not return a JSON report. '
  ...
  for index = 1, math.min(#report, MAX_RECORDS) do
    if #diagnostics >= MAX_DIAGNOSTICS then
      diagnostics[#diagnostics + 1] =
        status(context, 'diagnostic-limit', 'Only the first 512 markuplint diagnostics are shown.')
      break
    end
    ...
    elseif canonical(finding.filePath, cwd) == target and finding.severity ~= 'off' then
      ...
      local item = {
        ...
        source = SOURCE,
        code = nonempty(finding.ruleId),
        message = clean(finding.message),
        user_data = {
          filename = finding.filePath,
          rule = nonempty(finding.ruleId),
      ...
      if col and raw and #raw <= MAX_TEXT then
        ...
        if last then
          local actual = {}
          for row = lnum, end_lnum do
            actual[#actual + 1] = line_at(row)
          end
  ...
  if #report > MAX_RECORDS then
    diagnostics[#diagnostics + 1] =
      status(context, 'record-limit', 'markuplint report exceeded the 10000-record parser limit.')
  end
  return diagnostics
end
