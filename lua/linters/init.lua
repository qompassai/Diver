...
local function load_linter(name, module_name)
  ...
  if type(result) ~= 'table' then
    M.load_errors[name] = ('%s returned %s; its file must end with `return { ... }`'):format(module_name, type(result))
    return
  end
  ---@cast result Linter
  ...
  job = vim.system(command, system_options, function(result)
    vim.schedule(function()
      ...
      if not accepts_exit_code(definition, result.code) and #parsed == 0 then
        ...
        vim.notify(
          ('%s failed with exit code %s%s'):format(name, tostring(result.code), detail ~= '' and (': ' .. detail) or ''),
...
function M.info(bufnr, include_all)
  ...
  local lines = {
    ...
    ('Automatic linting: %s'):format(M.options.enabled and not vim.b[bufnr].lint_disabled and 'enabled' or 'disabled'),
