...
--
-- cfn-lint configuration precedence remains:
--
--   .cfnlintrc
--     <
--   template Metadata
...
--   CLI options
--
-- Therefore this adapter deliberately keeps CLI policy minimal.
--
-- Current Neovim diagnostics use 0-based rows and byte columns. cfn-lint's
-- JSON positions are 1-based, so all source positions are converted here.
...
local function finding_record(value)
  ...
  return string_value(value.Message) ~= nil
    and (
      type(value.Rule) == 'table'
      or type(value.Location) == 'table'
      or string_value(value.Id) ~= nil
    )
end
...
local function finding_diagnostic(
  ...
  if level ~= nil then
    ...
    diagnostic_severity = severity_from_rule(
      rule_id
    )
  end
  ...
  return {
    {
      ...
      message = string.format(
        'cfn-lint output exceeded the %d-byte parser limit',
        MAX_OUTPUT_BYTES
      ),
...
local function parse(
  ...
  assert(
    type(context) == 'table',
    'cfn-lint parser requires LintContext'
  )

  assert(
    type(context.bufnr) == 'number',
    'cfn-lint parser requires context.bufnr'
  )
...
local function arguments(context)
  assert(
    type(context) == 'table',
    'cfn-lint arguments require LintContext'
  )

  ---@type string[]
  local args = {
    --
    -- Native machine-readable output with full rule and range metadata.
    --
    '--format',
    'json',

    --
    -- Include informational diagnostics. Upstream leaves them disabled by
    ...
    --
    -- We parse diagnostics irrespective of exit status, so editor execution
    -- does not need non-zero severity policy.
    --
    '--non-zero-exit-code',
    'none',
  }
  ...
  if truthy(
    vim.env.NVIM_CFN_LINT_EXPERIMENTAL
  ) then
    args[#args + 1] =
      '--include-experimental'
  end

  local regions = split_csv(
    string_value(
      vim.env.NVIM_CFN_LINT_REGIONS
    )
  )
  ...
  --
  -- "-" means CloudFormation template contents are read from stdin.
  --
  -- This allows diagnostics to reflect unsaved Neovim changes while config
  -- discovery still occurs relative to cwd.
  --
  args[#args + 1] = '-'
...
return {
  ...
  --
  -- cfn-lint is substantial schema/rule validation, but still appropriate
  -- for lint-on-save. It should not run on every InsertChanged event unless
  -- your central scheduler debounces aggressively.
  --
  automatic = true,
  ...
  --
  -- cfn-lint normally uses severity bitmasks for exit status:
  --
  --   2  error
  --   4  warning
  --   8  informational
  --
  -- We additionally pass --non-zero-exit-code none, but keeping this true
  -- prevents diagnostic loss if syntax/runtime behavior differs.
  --
