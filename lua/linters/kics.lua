--- kics linter adapter — infrastructure-as-code security checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `kics` program and turn its complaints into squiggles under your code. It only runs when
--- linting is triggered (usually on save), and only if `kics` is installed on your computer.
---@module 'linters.kics'
-- #################################################################
...
local function parse(output, context)
    ...
    then
        diagnostics[#diagnostics + 1] = status_diagnostic(
            context,
            'partial-scan',
            'KICS reported scan or query failures; these results are incomplete.'
        )
    end
    if report.files_scanned == 0 or report.files_parsed == 0 or report.queries_total == 0 then
        diagnostics[#diagnostics + 1] = status_diagnostic(
            context,
            'empty-scan',
            'KICS scanned no files, parsed no files, or ran no '
                .. 'queries. Check supported platforms, exclusions and query assets.'
        )
    end
    if limited then
        diagnostics[#diagnostics + 1] = status_diagnostic(
