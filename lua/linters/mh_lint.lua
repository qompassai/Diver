--- mh_lint linter adapter — miss_hit MATLAB style checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `mh_lint` program and turn its complaints into squiggles under your code. It only runs
--- when linting is triggered (usually on save), and only if `mh_lint` is installed on your computer.
---@module 'linters.mh_lint'
-- #################################################################
...
local function parse(output, context)
    ...
    if context.modified then
        return {
            status_diagnostic(
                context,
                'save-required',
                'Save this buffer before running mh_lint; it reads files from disk.'
            ),
        }
    end
    ...
    if filename == nil then
        return {
            status_diagnostic(
                context,
                'filename-required',
                'Save this buffer as a MATLAB or Octave .m file before running mh_lint.'
            ),
        }
    end
    ...
    if #output > MAX_OUTPUT_BYTES then
        return {
            status_diagnostic(
                context,
                'output-limit',
                'mh_lint output exceeded the 16 MiB parser limit; results are incomplete.'
            ),
        }
    end
    ...
    if unexpected then
        diagnostics[#diagnostics + 1] = status_diagnostic(
            context,
            'unparsed-output',
            'mh_lint returned unexpected output. Check its arguments, '
                .. 'configuration and Python environment; results may be incomplete.'
        )
    elseif not completed and #diagnostics == 0 and not limited then
        diagnostics[#diagnostics + 1] = status_diagnostic(
            context,
            'missing-summary',
            'mh_lint returned no completion summary or current-file diagnostics. Check the executable and timeout.'
        )
    end
    ...
    if limited then
        diagnostics[#diagnostics + 1] = status_diagnostic(
            context,
            'result-limit',
            'mh_lint reached the editor parser limit; run the CLI for the complete report.'
        )
    end
...
local function arguments(context)
    ...
    local args = {
        ...
        '--single',
        '--input-encoding',
        string_value(vim.env.NVIM_MH_LINT_ENCODING) or 'utf-8',
    }
