...
local function parse(output, context)
  ...
  if context.modified then
    return {
      status_diagnostic(context, 'save-required', 'Save this buffer before running mh_lint; it reads files from disk.'),
    }
  end
  ...
  if unexpected then
    diagnostics[#diagnostics + 1] = status_diagnostic(
      context,
      'unparsed-output',
      'mh_lint returned unexpected output. Check its arguments, configuration and Python environment; results may be incomplete.'
    )
