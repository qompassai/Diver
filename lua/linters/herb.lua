--- herb linter adapter — HTML+ERB template checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `herb` program and turn its complaints into squiggles under your code. It only runs when
--- linting is triggered (usually on save), and only if `herb` is installed on your computer.
---@module 'linters.herb'
...
local function position(value, context)
    if type(value) ~= 'table' or not integer(value.line) or value.line < 1 or not integer(value.column) then
        return nil
    end
...
local function parse(output, context)
    ...
    if not ok or type(report) ~= 'table' or type(report.offenses) ~= 'table' or not vim.islist(report.offenses) then
        return {
            status_diagnostic(
                context,
                'invalid-report',
                'Herb returned no valid JSON report. Check exclusions, '
                    .. 'configuration, custom rules and executable compatibility.'
            ),
        }
    end
    local result = {}
    ...
    for index = 1, math.min(#report.offenses, MAX_RESULTS) do
        ...
        if type(issue) ~= 'table' or not string_value(issue.filename) or not string_value(issue.message) then
            malformed = true
        elseif canonical_path(issue.filename, root) ~= target then
            ...
            if type(location.start) == 'table' then
                ...
                item.user_data.reported_column = integer(location.start.column) and location.start.column or nil
            end
            if type(location['end']) == 'table' then
                item.user_data.reported_end_line = integer(location['end'].line) and location['end'].line or nil
                item.user_data.reported_end_column = integer(location['end'].column) and location['end'].column or nil
            end
            if exact and end_exact and end_row and row and (end_row > row or (end_row == row and end_col >= col)) then
    ...
    if malformed then
        result[#result + 1] =
            status_diagnostic(context, 'malformed-result', 'Some Herb results could not be interpreted completely.')
    end
    if limited then
        result[#result + 1] = status_diagnostic(
            context,
