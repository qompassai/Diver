...
local function severity(level, kind)
    ...
    if numeric ~= nil then
        local mapped = level_severities[floor(numeric)]
    ...
    if type(kind) == 'string' then
        local mapped = type_severities[kind:upper()]
...
---@param root string
---@return boolean
local function belongs_to_buffer(source, filename, root)
    --
...
local function diagnostic_from_entry(entry, context, filename, root)
    if not belongs_to_buffer(entry.source, filename, root) then
        return nil
    end
    ...
    -- structural error.
    --
    local lnum = max(integer(entry.line, 1) - 1, 0)

    local message = string_or(entry.message, 'reStructuredText lint violation')
    ...
    return {
        ...
        --
        col = 0,
        end_col = end_column(lnum, context.bufnr),

        message = message,

        severity = severity(entry.level, kind),
...
local function parse(output, context)
    ...
    assert(type(context) == 'table', 'restructuredtext-lint parser requires a LintContext')
    ...
    assert(#output <= OUTPUT_LENGTH_MAX, 'restructuredtext-lint output exceeded maximum size')
    ...
    local count = min(#decoded, DIAGNOSTICS_MAX)
    ...
    for index = 1, count do
        ...
        if type(raw) == 'table' then
            ...
            local entry = diagnostic_from_entry(raw, context, filename, root)
