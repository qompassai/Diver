--- markdownlint-cli2 linter adapter — Markdown style checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `markdownlint-cli2` program and turn its complaints into squiggles under your code. It
--- only runs when linting is triggered (usually on save), and only if `markdownlint-cli2` is installed on your
--- computer.
---@module 'linters.markuplint_cli2'
-- #################################################################
...
local function parse(output, context)
    assert(type(context) == 'table' and type(context.bufnr) == 'number', 'markdownlint-cli2 requires LintContext')
    ...
    if not ok or type(report) ~= 'table' or not vim.islist(report) or not output:match('^%s*%[') then
        return {
            status(
                context,
                'invalid-report',
                'No valid JSON array from markdownlint-cli2. Install the companion config '
                    .. 'and stdout reporter; check CLI stderr for configuration or startup errors.'
            ),
        }
    end
    ...
    for index = 1, math.min(#report, MAX_RECORDS) do
        ...
        if type(item) ~= 'table' or not nonempty(item.ruleDescription) or not nonempty(item.fileName) then
            malformed = true
        elseif item.fileName ~= 'stdin' and fs.normalize(item.fileName) ~= fs.joinpath(cwd, 'stdin') then
            foreign = true
        else
            local valid_row = integer(item.lineNumber) and item.lineNumber > 0 and item.lineNumber <= count
            local row = valid_row and item.lineNumber - 1 or 0
            local col, finish = 0, nil
            local range = item.errorRange
            if valid_row and type(range) == 'table' and integer(range[1]) and range[1] > 0 and integer(range[2]) then
    ...
    if limited then
        diagnostics[#diagnostics + 1] = status(context, 'result-limit', 'Only the first 512 diagnostics are shown.')
    end
    return diagnostics
end
