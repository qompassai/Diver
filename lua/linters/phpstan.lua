...
local function parse(output, context)
        ...
        then
            ...
            for _, issue in ipairs(group.messages) do
                ...
                if
                    ...
                    or (issue.line ~= vim.NIL and issue.line ~= nil and (not integer(issue.line) or issue.line < 1))
                then
                    malformed = true
    ...
    if report.totals.errors ~= #report.errors or (not limited and file_errors ~= report.totals.file_errors) then
        malformed = true
    end
    if foreign then
        result[#result + 1] = status(
            context,
            'other-files',
            'PHPStan reported errors outside this buffer, possibly a trait context. '
                .. 'Run project analysis to inspect them.'
        )
    end
    if malformed then
        result[#result + 1] =
