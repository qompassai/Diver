--- checkcode linter adapter — kernel-style patch checker wiring.
---
--- Plain-language version: a linter is like a spell-checker, but for code instead of words. This file teaches
--- Neovim how to run the `checkcode` program and turn its complaints into squiggles under your code. It only runs
--- when linting is triggered (usually on save), and only if `checkcode` is installed on your computer.
---@module 'linters.checkcode'
-- #################################################################
...
-- #################################################################
---@source https://www.mathworks.com/help/matlab/ref/checkcode.html
