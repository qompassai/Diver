...
function M.setup()
  ...
  map("n", "<leader>dl", vim.diagnostic.setloclist, vim.tbl_extend("force", opts, { desc = "Show buffer diagnostics" }))
  ...
  vim.keymap.set("n", "<leader>dq", vim.diagnostic.setqflist, { desc = "Show project diagnostics" })
  ...
  map(
    ...
    { desc = "Trouble [T]oggle LSP [w]indow" }
  )
  ...
  -- Step over
  map("n", "<leader>DS", "<cmd>lua require'dap'.step_over()<CR>", { desc = "Nvim-Dap Step over" })
  -- In normal mode, press 'Space' + 'D' + 'S' to step over
