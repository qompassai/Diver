...
---@field root string? Project root being indexed.                         ---@field started_at integer High-resolution start timestamp from vim.uv.hrtime().
...
---
---The active `vim.SystemObj` is the authoritative indicator of whether an ---indexing operation is in progress.
---
---@return boolean
function M.running()                                                               return M.current.job ~= nil
end
...
---`vim.SystemObj`, allowing the rest of the SCIP subsystem to inspect,    ---cancel, or report on the active job.
---
...
---@return nil                                                             function M.start(job, indexer, root, started_at)
        M.current.job = job
        M.current.indexer = indexer                                                M.current.root = vim.fs.normalize(root)
