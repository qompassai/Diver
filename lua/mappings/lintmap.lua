...
local function runners(bufnr)
    ...
    ---@class LintmapLintersModule
    ---@field linters_by_ft table<string, string[]>
    ---@field run? fun(bufnr: integer, opts?: table)
    ---@field reset? fun(bufnr: integer)
    ---@field info? fun(bufnr: integer, flag: boolean)
    ---@field stop? fun(bufnr: integer)
    ---@type LintmapLintersModule?
    local lint = package.loaded.linters
    ---@class LintmapFormattersModule
    ---@field formatters_by_ft table<string, string[]>
    ---@field format? fun(opts?: table, callback?: fun(result: table)): table
    ---@field info? fun(bufnr: integer, flag: boolean)
    ---@field stop? fun(bufnr: integer)
    ---@type LintmapFormattersModule?
...
local function attach(bufnr)
    ...
    if core.source(bufnr) then
        ...
        if vim.fn.exists(':LinterUpdate') == 2 then
            maps[#maps + 1] = {
                lhs = '<LocalLeader>cu',
...
function M.setup()
    ...
    core.install(OWNER, 0, {
        ...
        {
            lhs = '<Leader>xc',
            desc = 'Run config tiger check',
            rhs = core.command('ConfigTigerCheck'),
        },
        {
            lhs = '<Leader>xo',
            desc = 'Open config tiger check log',
            rhs = core.command('ConfigTigerCheckLog'),
        },
        {
            lhs = '<Leader>xw',
            desc = 'Send workspace diagnostics to quickfix',
            rhs = core.command('DiagnosticsWorkspace'),
        },
    })
end
