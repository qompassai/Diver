...
local function start_termdebug(bufnr)
    ...
    vim.ui.input({ prompt = 'Executable for GDB: ', completion = 'file' }, function(path)
        ...
        api.nvim_cmd({ cmd = 'Termdebug', args = { path }, magic = { file = false, bar = false } }, {})
    end)
end
...
function M.setup(opts)
    ...
    -- The old <leader>hl/hs/hy config self-check maps are gone: <M-h> is owned
    -- by cicdmap's terminal toggle and any <M-h>? second key is a strict-prefix
    -- conflict. The config-health verbs now live on lintmap's <M-x>c/o/w.
