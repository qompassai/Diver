...
function M.notify(message, level)
    vim.notify(tostring(message), level or vim.log.levels.WARN, { title = 'Native mappings' })
end
...
-- Delete only registrations still owned by us. A later user's replacement survives.
        ...
        for _, owned in ipairs(previous) do
            for _, current in ipairs(mappings(scope, owned.mode)) do
                if matches(current, owned) then
                    local opts = scope ~= 0 and { buffer = scope } or {}
...
function M.install(owner, scope, definitions)
    ...
    for _, entry in ipairs(ordered) do
        ...
        local modes = type(entry.mode) == 'table' and entry.mode or { entry.mode or 'n' }
        for _, mode in ipairs(modes) do
            ...
            if occupied then
                ...
                vim.keymap.set(mode, entry.lhs, entry.rhs, opts)
                for _, current in ipairs(mappings(scope, mode)) do
                    if keys(current.lhs) == key then
                        table.insert(state.scopes[scope], {
                            mode = mode,
                            key = key,
                            desc = current.desc,
...
-- Refresh loaded buffers too; setup need not precede the first FileType/LspAttach.
function M.watch(owner, attach, events)
    ...
    api.nvim_create_autocmd(events or { 'BufEnter', 'FileType', 'LspAttach', 'LspDetach' }, {
        group = state.group,
...
-- Bind callbacks to the originating buffer even when UI selection is asynchronous.
