...
-- Rename symbol
map("n", "<leader>ra", vim.lsp.buf.rename, vim.tbl_extend("force", bufopts, { desc = "Nvim-LSP [r]ename [s]ymbol" }))
-- In normal mode, press 'Space' + 'r' + 'a' to rename the symbol under the cursor.

-- Code action
map({ "n", "v" }, "<leader>Ca", vim.lsp.buf.code_action, vim.tbl_extend("force", bufopts, { desc = "Nvim-LSP [C]ode [a]ction" }))
...
map("n", "<leader>jr", "<cmd>IronRepl<CR>", vim.tbl_extend("force", bufopts, { desc = "Iron [j]upyter [r]epl ipython" }))
...
map("n", "<leader>je", ":MoltenInit<CR>", vim.tbl_extend("force", bufopts, { desc = "Molten [j]upyter [e]nvironment start" }))
