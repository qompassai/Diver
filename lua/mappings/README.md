...
Inventory of the ten supplied Lua files, reviewed on 2026-09-21. **114 mapping definitions** are documented, including disabled mappings, overlapping definitions, and definitions that the supplied loader does not activate. A definition with multiple modes occupies one row; these are not 114 unique effective shortcuts.

Files are alphabetical. Within each mapping table, keys are sorted case-insensitively by their written notation, then by exact spelling and mode; punctuation appears before letters. Uppercase remains significant: `<leader>ds` and `<leader>dS` are different shortcuts. Tables preserve the original `desc` text; the behavior column clarifies misleading descriptions.

This is a static source audit, not a live Neovim session test. No mapping files were changed. `mojomap.lua`, `navmap.lua`, and `utilmap.lua` are referenced by the loader but were not supplied, so their mappings and possible conflicts are unknown. No SCIP binding appears in these ten files; proposed SCIP keys below are reservations, not existing functionality.
...
| 11 | `navmap` | Referenced; not supplied |
...
The prefix choices below are a proposed personal convention, not a Neovim standard. Check the three missing mapping modules before claiming any key is free. No proposed key has been installed.
...
| Navigation | `<leader>z` | Retain Zoxide ownership, with distinct leaves for global/window/tab query and interactive actions. |
...
Complete the correctness fixes before changing muscle memory. The priorities are sequential; checklist items within each priority are alphabetical by their bold label. These are recommendations from source review, not applied fixes.
...
- [ ] **Missing inventory:** Add `mojomap.lua`, `navmap.lua`, `utilmap.lua`, core LSP attach integration, and any DAP/SCIP modules that create their own keys before calling the inventory complete for the whole configuration.
