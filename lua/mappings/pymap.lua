...
function M.setup_pymap()
    ...
    vim.api.nvim_create_autocmd('LspAttach', {
        callback = function(ev)
            ...
            end, vim.tbl_extend('force', bufopts, { desc = '[d]ebug [p]ython [m]ethod' }))
            map('n', '<leader>dpc', function()
                require('dap-python').test_class()
            end, vim.tbl_extend('force', opts, { desc = '[d]ebug [p]ython [c]lass' }))
            map('n', '<leader>dps', function()
                require('dap-python').debug_selection()
            end, vim.tbl_extend('force', opts, { desc = '[d]ebug [p]ython [s]election' }))
            map(
                'n',
                '<leader>pd',
            ...
            map(
                ...
                vim.tbl_extend('force', opts, { desc = '🐍 [p]ython find [r]eferences' })
            )
            map(
                'n',
                '<leader>pgi',
                vim.lsp.buf.implementation,
                vim.tbl_extend('force', opts, { desc = '🐍 [p]ython [g]o to [i]mplementation' })
            )
            map('n', '<leader>pl', function()
                vim.cmd('PythonLint')
            end, vim.tbl_extend('force', opts, { desc = '[p]ython [l]int' }))

            map('n', '<leader>ptF', function()
                vim.cmd('PyTestFile')
            end, vim.tbl_extend('force', opts, { desc = '🐍 [p]ython [t]est [F]ile' }))
            map('n', '<leader>ptf', function()
                vim.cmd('PyTestFunc')
            end, vim.tbl_extend('force', opts, { desc = '🐍 [p]ython [t]est [f]unction' }))
            ...
            end, vim.tbl_extend('force', opts, { desc = '🐍 [p]ython [p]oetry Update' }))
            ...
            map(
                ...
                vim.tbl_extend('force', bufopts, { desc = '[j]upyter [c]lear REPL output' })
            )
            -- In normal mode, press 'Space' + 'j' + 'c' to clear the current REPL output.

            map('n', '<leader>jel', function()
                vim.cmd('MoltenEvaluateLine')
            end, vim.tbl_extend('force', opts, { desc = '[j]upter [e]valuate [l]ine' }))
            ...
            map(
                ...
                vim.tbl_extend('force', bufopts, { desc = '[j]upyter [i]nterrupt kernel' })
            )
            ...
            end, vim.tbl_extend('force', opts, { desc = '[j]upyter [r]evaluate [c]ell' }))
            ...
            map(
                ...
                '<leader>jrs',
                '<cmd>IronReplSend<CR>',
                vim.tbl_extend('force', bufopts, { desc = '[j]upyter [r]epl code' })
            )
            ...
            map(
                ...
                vim.tbl_extend('force', bufopts, { desc = '[j]upyter [s]tart [k]ernel' })
            )
            ...
            map(
                ...
                vim.tbl_extend('force', bufopts, { desc = '🦦 [j]upter [o]utput toggle panel' })
            )
            ...
            map(
                ...
                vim.tbl_extend('force', bufopts, { desc = '[j]upyter [t]erminal [m]ark' })
            )
            ...
            map(
                ...
                vim.tbl_extend('force', bufopts, { desc = '[j]upyter [t]erminal [s]et' })
            )
            ...
            map('n', '<leader>ue', '<cmd>UVEnvironment<CR>', vim.tbl_extend('force', opts, { desc = 'UV Environment' }))
            map('n', '<leader>ui', '<cmd>UVInit<CR>', vim.tbl_extend('force', opts, { desc = 'UV Initialize' }))
            map('n', '<leader>ua', '<cmd>UVAdd<CR>', vim.tbl_extend('force', opts, { desc = 'UV Add Package' }))
            map('n', '<leader>ud', '<cmd>UVRemove<CR>', vim.tbl_extend('force', opts, { desc = 'UV Remove Package' }))
            map('n', '<leader>uc', '<cmd>UVSync<CR>', vim.tbl_extend('force', opts, { desc = 'UV Sync Packages' }))
        end,
    })
end
